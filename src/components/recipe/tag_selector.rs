    ...
    rsx! {
        div { class: "space-y-3",
            div { class: "flex items-center justify-between",
                ...
                span { class: "text-xs text-muted-foreground", "{tags.len()}/{max_tags}" }
            }
            if !tags.is_empty() {
                div { class: "flex flex-wrap gap-2",
                    for tag in tags.iter() {
                        RecipeTagChipEditable { tag: tag.clone(), on_remove: remove_tag }
                    }
                }
            }
            if can_add {
                div { class: "relative",
                    ...
                    if *show_suggestions.read() && !results.is_empty() {
                        div { class: "absolute z-50 w-full mt-1 py-1 bg-popover border border-border rounded-lg shadow-lg max-h-60 overflow-y-auto",
                            for tag in results.iter() {
                                {
                                    ...
                                    rsx! {
                                        button {
                                            ...
                                            span { "{tag_name}" }
                                            if is_selected {
                                                span { class: "ml-auto text-xs text-muted-foreground", "Added" }
                                            }
                                        }
                                    }
                                }
            ...
            if *show_browse.read() && can_add {
                div { class: "border border-border rounded-lg p-4 space-y-4 max-h-80 overflow-y-auto",
