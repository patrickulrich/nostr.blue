    ...
    if clickable {
        rsx! {
            Link {
                ...
                span { "{display_text}" }
                if let Some(c) = count {
                    span { class: "text-xs opacity-70", "({c})" }
                }
            }
        }
    } else {
        rsx! {
            span { class: "inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-medium",
                span { "{display_text}" }
                if let Some(c) = count {
                    span { class: "text-xs opacity-70", "({c})" }
                }
...
pub fn RecipeTagChipEditable(tag: String, on_remove: EventHandler<String>) -> Element {
    ...
    rsx! {
        span { class: "inline-flex items-center gap-1 pl-3 pr-1 py-1 rounded-full bg-primary/10 text-primary text-sm font-medium",
            span { "{display}" }
            button {
                r#type: "button",
    ...
    if clickable {
        rsx! {
            Link {
                ...
                if let Some(e) = emoji {
                    span { class: "text-sm leading-none", "{e}" }
                }
                span { "{tag}" }
                if let Some(c) = count {
                    span { class: "text-xs text-muted-foreground font-normal", "· {c}" }
                }
            }
        }
    } else {
        rsx! {
            span { class: "inline-flex items-center gap-1.5 rounded-full px-3 py-2 bg-muted text-sm font-medium h-9",
                if let Some(e) = emoji {
                    span { class: "text-sm leading-none", "{e}" }
                }
                span { "{tag}" }
