            ...
            Link {
                ...
                div { class: "relative bg-black cursor-pointer group",
                    ...
                    if stream_meta.status == StreamStatus::Live {
                        div { class: "absolute top-2 left-2 bg-red-600 text-white text-xs font-bold px-3 py-1 rounded flex items-center gap-1",
                            div { class: "w-2 h-2 bg-white rounded-full animate-pulse" }
                            "LIVE"
                        }
                    } else if stream_meta.status == StreamStatus::Planned {
                        div { class: "absolute top-2 left-2 bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded",
                            "UPCOMING"
                        }
                    }
                    if stream_meta.status == StreamStatus::Live {
                        if let Some(viewers) = stream_meta.current_participants {
                            div { class: "absolute bottom-2 right-2 bg-black/75 text-white text-xs px-2 py-1 rounded flex items-center gap-1",
                                svg {
                                    class: "w-3 h-3",
                                    xmlns: "http://www.w3.org/2000/svg",
                                    fill: "none",
            ...
            div { class: "p-4",
                ...
                if let Some(summary_text) = &stream_meta.summary {
                    if !summary_text.is_empty() {
                        p { class: "text-sm text-muted-foreground whitespace-pre-wrap line-clamp-2",
                            "{summary_text}"
                        }
                    }
                }
