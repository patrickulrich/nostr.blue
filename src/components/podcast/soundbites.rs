...
pub fn PodcastSoundbites(props: PodcastSoundbitesProps) -> Element {
    ...
    rsx! {
        div { class: "space-y-2",
            div { class: "flex items-center gap-2 text-sm font-medium text-muted-foreground pb-2",
                span { class: "w-4 h-4", dangerous_inner_html: icons::MUSIC_NOTE }
    ...
    if props.compact {
        rsx! {
            div { class: "flex items-center gap-2 text-sm",
                ...
                span { class: "truncate flex-1", "{title}" }
                span { class: "text-xs text-muted-foreground font-mono", "{duration_str}" }
            }
        }
    } else {
        rsx! {
            div { class: "{base_class}",
                button {
                    ...
                    onclick: handle_play.clone(),
                    title: "Play clip",
                    span { dangerous_inner_html: if *is_playing.read() { icons::PAUSE } else { icons::PLAY } }
                }
                div { class: "flex-1 min-w-0",
                    div { class: "font-medium text-sm truncate", "{title}" }
                    div { class: "flex items-center gap-2 text-xs text-muted-foreground",
                        span { class: "font-mono", "at {start_str}" }
                        span { "•" }
                        span { "{duration_str} clip" }
                    }
                }
                if *is_playing.read() {
...
pub fn SoundbiteGrid(props: SoundbiteGridProps) -> Element {
    if props.soundbites.is_empty() {
        return rsx! {
            div { class: "text-center py-8 text-muted-foreground", "No soundbites to display." }
        };
    }
    ...
    rsx! {
        div { class: "{grid_class}",
            for (idx , item) in props.soundbites.iter().enumerate() {
                SoundbiteGridCard { key: "{idx}", item: item.clone() }
            }
        }
    }
}
        ...
        div {
            ...
            div { class: "relative p-4",
                div { class: "flex items-center justify-center mb-3",
                    div { class: "h-12 w-12 rounded-full bg-primary/90 group-hover:bg-primary flex items-center justify-center text-primary-foreground transition shadow-lg",
                        span { dangerous_inner_html: if *is_playing.read() { icons::PAUSE } else { icons::PLAY } }
                    }
                }
                div { class: "font-semibold text-sm truncate mb-1", "{title}" }
                div { class: "text-xs text-muted-foreground truncate mb-1", "{item.episode_title}" }
                div { class: "flex items-center justify-between text-xs text-muted-foreground",
                    span { class: "truncate", "{item.podcast_title}" }
                    span { class: "font-mono shrink-0", "{duration_str}" }
                }
            }
            if *is_playing.read() {
...
pub fn FeaturedSoundbite(props: FeaturedSoundbiteProps) -> Element {
    ...
    rsx! {
        div { class: "relative rounded-xl overflow-hidden bg-gradient-to-br from-primary/20 to-primary/5 border border-primary/20",
            div { class: "flex items-center gap-4 p-6",
                div { class: "relative w-24 h-24 rounded-lg overflow-hidden bg-muted shrink-0 shadow-lg",
                    ...
                    button {
                        ...
                        onclick: handle_play,
                        div { class: "h-12 w-12 rounded-full bg-primary flex items-center justify-center text-primary-foreground shadow-lg",
                            span { dangerous_inner_html: if *is_playing.read() { icons::PAUSE } else { icons::PLAY } }
                        }
                    }
                }
                div { class: "flex-1 min-w-0",
                    div { class: "inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/20 text-primary text-xs font-medium mb-2",
                        span {
                            class: "w-3 h-3",
                    ...
                    h3 { class: "font-bold text-lg truncate mb-1", "{title}" }
                    p { class: "text-sm text-muted-foreground truncate mb-1", "{props.episode_title}" }
                    div { class: "flex items-center gap-2 text-xs text-muted-foreground",
