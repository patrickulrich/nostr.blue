        ...
        rsx! {
            div { class: "space-y-4",
                if let Some(ref title) = props.title {
                    h3 { class: "font-semibold text-sm text-muted-foreground", "{title}" }
                }
                if !hosts.is_empty() {
                    PersonGroup {
                        title: "Hosts",
    ...
    rsx! {
        div { class: "space-y-2",
            div { class: "flex items-center gap-2 text-xs font-medium text-muted-foreground uppercase tracking-wide",
                span { class: "w-4 h-4", dangerous_inner_html: props.icon }
                "{props.title}"
            }
            div { class: "grid grid-cols-1 sm:grid-cols-2 gap-2",
                for (idx , person) in props.persons.iter().enumerate() {
                    PersonCard { key: "{idx}", person: person.clone() }
                }
            }
        }
    }
    ...
    if props.compact {
        rsx! {
            div { class: "flex items-center gap-2",
                PersonAvatar { person: person.clone(), size: AvatarSize::Small }
                span { class: "text-sm font-medium truncate", "{person.name}" }
            }
        }
    } else {
        rsx! {
            div { class: "flex items-center gap-3 p-2 rounded-lg bg-muted/50 hover:bg-muted transition",
                PersonAvatar { person: person.clone(), size: AvatarSize::Medium }
                div { class: "flex-1 min-w-0",
                    if has_link {
                        ...
                        span { class: "font-medium text-sm truncate block", "{person.name}" }
                    }
                    if !role_display.is_empty() {
                        span { class: "inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-primary/10 text-primary capitalize",
                            "{role_display}"
                        }
                    }
                }
    ...
    rsx! {
        div {
            ...
            if let Some(ref img_url) = person.img {
                ...
                div { class: "w-full h-full flex items-center justify-center {color_class} {text_class} font-semibold",
                    "{initials}"
                }
            }
        }
    ...
    rsx! {
        div { class: "flex items-center",
            div { class: "flex -space-x-2",
                ...
                if overflow > 0 {
                    div { class: "{size_class} rounded-full flex items-center justify-center bg-muted ring-2 ring-background {text_class} font-medium text-muted-foreground",
                        "+{overflow}"
                    }
                }
            }
...
pub fn FeaturedPerson(props: FeaturedPersonProps) -> Element {
    ...
    rsx! {
        div { class: "flex items-start gap-4 p-4 rounded-xl bg-muted/50",
            PersonAvatar { person: person.clone(), size: AvatarSize::Large }
            div { class: "flex-1",
                h3 { class: "font-bold text-lg", "{person.name}" }
                span { class: "inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-primary/10 text-primary capitalize mb-2",
                    "{role_display}"
                }
...
pub fn InlineCredits(props: InlineCreditsProps) -> Element {
    ...
    rsx! {
        div { class: "flex items-center gap-2 text-xs text-muted-foreground",
            span { class: "w-3 h-3", dangerous_inner_html: icons::MIC }
            span { class: "truncate", "with {display}" }
        }
    }
}
