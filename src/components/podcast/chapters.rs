...
pub fn PodcastChapters(props: PodcastChaptersProps) -> Element {
    ...
    match &*chapters_read {
        ...
        Some(Err(e)) => {
            ...
            drop(chapters_read);
            rsx! {
                div { class: "text-xs text-muted-foreground p-2", "Failed to load chapters: {err_msg}" }
            }
        }
        None => {
            drop(chapters_read);
...
fn ChapterItem(props: ChapterItemProps) -> Element {
    ...
    rsx! {
        div { class: "{base_class}", onclick: handle_click,
            if !props.compact {
            ...
            div { class: "flex-1 min-w-0",
                div { class: if props.is_current { "font-medium text-sm text-primary" } else { "font-medium text-sm" },
            ...
            div { class: "text-xs text-muted-foreground shrink-0 font-mono", "{timestamp}" }
            if props.is_current {
                div { class: "text-primary", dangerous_inner_html: icons::PLAY }
            }
        }
    }
}
