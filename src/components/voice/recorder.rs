                ...
                match js_sys::eval(&script) {
                    Ok(result) => {
                        if result.as_bool() == Some(true) {
                            ...
                            log::error!(
                                "Preview toggle failed - audio element not found or blob URL not ready"
                            );
                        }
                    }
                    Err(e) => {
            ...
            spawn(async move {
                ...
                match js_sys::eval(&get_result_script) {
                    Ok(result) => {
                        if result.is_null() || result.is_undefined() {
                            log::error!(
                                "getResult returned null/undefined for recorder: {}",
                                recorder_id
                            );
                            return;
                        }
        ...
        div { class: "bg-muted/30 rounded-lg p-6 space-y-4",
                ...
                match state.read().clone() {
                    ...
                    RecorderState::Recording => rsx! {
                        div { class: "space-y-2",
                            ...
                            div { class: "text-2xl font-mono", "{format_time(*current_time.read())}" }
                            div { class: "text-sm text-muted-foreground", "Time remaining: {format_time(remaining_time)}" }
                    ...
                    RecorderState::Stopped { duration } => rsx! {
                        div { class: "text-green-600", "Recording complete: {format_time(duration)}" }
                    },
            ...
            div { class: "h-20 bg-muted rounded flex items-center justify-center",
                match state.read().clone() {
                    ...
                    _ => rsx! {
                        div { class: "text-muted-foreground", "Waveform will appear here" }
                    },
                }
            }
                ...
                match state.read().clone() {
                    RecorderState::Idle | RecorderState::Error { .. } => rsx! {
                        button {
                            ...
                            svg { class: "w-8 h-8", view_box: "0 0 24 24", fill: "currentColor",
