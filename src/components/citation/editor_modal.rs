...
pub fn CitationEditorModal(mut props: CitationEditorModalProps) -> Element {
    ...
    // Internal citation fields (Kind 30)
    let mut coordinate = use_signal(String::new); // nostr address being cited

    // External web fields (Kind 31)
    let mut url = use_signal(String::new);

    // Hardcopy fields (Kind 32)
    let mut page_range = use_signal(String::new);
    let mut publisher = use_signal(String::new);
    let mut doi = use_signal(String::new);

    // Prompt fields (Kind 33)
        ...
        move |(is_shown, citation)| {
            if is_shown {
                if let Some(ref cached) = citation {
                    // Set tab based on citation type
    ...
    rsx! {
        ...
        div {
            ...
            div {
                ...
                // Tab bar
                div {
                    class: "flex border-b border-border px-4",

                    {[CitationEditorTab::Internal, CitationEditorTab::External, CitationEditorTab::Hardcopy, CitationEditorTab::Prompt]
                        .iter()
                        .map(|tab| {
                            ...
                            rsx! {
                                button {
                                    ...
                                    class: if is_active {
                                        ...
                                        "flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 border-transparent text-muted-foreground hover:text-foreground transition-colors"
                                    },
                                    onclick: move |_| active_tab.set(tab_clone),

                                    // Tab icon
                                    {match tab {
                                        CitationEditorTab::Internal => rsx! { Link2Icon { class: "w-4 h-4".to_string() } },
                                        CitationEditorTab::External => rsx! { GlobeIcon { class: "w-4 h-4".to_string() } },
                                        CitationEditorTab::Hardcopy => rsx! { BookOpenIcon { class: "w-4 h-4".to_string() } },
                                        CitationEditorTab::Prompt => rsx! { SettingsIcon { class: "w-4 h-4".to_string() } },
                                    }}
                ...
                div {
                    ...
                    // Common fields (title and author - shown for all types)
                    div {
                        class: "grid grid-cols-2 gap-3",
