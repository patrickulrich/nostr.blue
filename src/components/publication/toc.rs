...
pub fn PublicationTocDynamic(
    ...
    rsx! {
        nav { class: "publication-toc",
            ...
            div { class: "px-3 py-2 border-b border-border",
                h2 { class: "font-semibold text-sm text-foreground truncate", "{current_title}" }
                p { class: "text-xs text-muted-foreground mt-0.5", "{item_count} items" }
            }
            div { class: "py-2 overflow-y-auto max-h-[calc(100vh-14rem)]",
                DynamicTocList {
                    key: "{current_title}",
...
fn DynamicTocList(
    ...
    if addresses_key != *last_key.read() {
        ...
        spawn(async move {
            ...
            let mut address_order: std::collections::HashMap<String, usize> = addrs
                .iter()
                .enumerate()
                .map(|(i, a)| (a.clone(), i))
                .collect();
    ...
    if *loading.read() {
        rsx! {
            div { class: "px-2 space-y-1",
                for _ in 0..5 {
                    div { class: "h-8 bg-muted/50 rounded animate-pulse" }
                }
            }
        }
    } else {
    ...
    rsx! {
        li {
            button {
                ...
                span { class: "flex-1 text-sm truncate", "{section.title}" }
                if is_index && child_count > 0 {
                    span { class: "text-xs text-muted-foreground", "{child_count}" }
...
pub fn PublicationTocSkeleton() -> Element {
    rsx! {
        nav { class: "publication-toc animate-pulse",
            div { class: "px-3 py-2 border-b border-border",
                div { class: "h-4 bg-muted rounded w-3/4" }
                div { class: "h-3 bg-muted rounded w-1/3 mt-1" }
            }
            div { class: "py-2 space-y-2 px-2",
                for _ in 0..8 {
