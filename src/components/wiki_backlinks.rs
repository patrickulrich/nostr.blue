...
pub fn WikiBacklinks(
    ...
    // Fetch backlinks if not provided
    use_effect(move || {
...
pub fn WikiBacklinksCompact(
    ...
    rsx! {
        div {
            class: "wiki-backlinks-compact {class}",
            h5 {
            ...
            div {
                ...
                for page in pages.read().iter() {
                    Link {
                        ...
                        to: Route::WikiDetail { identifier: page.article.identifier.clone() },
                        "{page.article.title}"
                    }
                }
                if *total.read() > limit {
                    span {
                        class: "inline-flex items-center px-2 py-1 text-xs text-muted-foreground",
...
pub fn WikiBacklinksGraph(
    ...
    rsx! {
        div {
            class: "wiki-backlinks-graph p-4 bg-card border border-border rounded-lg {class}",

            // Legend
            div {
                class: "flex items-center gap-4 mb-4 text-xs text-muted-foreground",
                div {
            ...
            div {
                ...
                // Backward links (left side)
                div {
                    class: "absolute left-0 top-0 bottom-0 w-1/3 flex flex-col justify-center gap-1 pr-4",
                ...
                div {
                    ...
                    div {
                        class: "px-4 py-2 bg-primary text-primary-foreground rounded-lg font-medium text-sm shadow-md",
                        "{identifier}"
                    }
                }

                // Forward links (right side)
                div {
                    class: "absolute right-0 top-0 bottom-0 w-1/3 flex flex-col justify-center gap-1 pl-4",
