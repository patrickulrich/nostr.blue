    ...
    rsx! {
        div { class: "relative",
            button {
                ...
                onclick: on_click,
                PinIcon { class: "w-4 h-4".to_string(), filled: *is_pinned.read() }
            }
            if pin_error.read().is_some() {
                span { class: "absolute top-full right-0 mt-1 text-xs text-destructive whitespace-nowrap",
                    "Pin failed"
                }
            }
        }
        ...
        div { class: "{bg_class}",
            div { class: "flex items-start gap-3 mb-3",
                ...
                div { class: "flex-1 min-w-0",
                    ...
                    p { class: "text-xs text-muted-foreground truncate", "{community.d_tag}" }
                }
                PinCommunityButton { a_tag: community.a_tag.clone() }
            }
            if let Some(desc) = &community.description {
                p { class: "text-sm text-muted-foreground mb-3 line-clamp-2", "{desc}" }
            }
...
pub fn CommunityCardSkeleton() -> Element {
    rsx! {
        div { class: "border border-border rounded-lg p-4 bg-card animate-pulse",
            div { class: "flex items-start gap-3 mb-3",
                div { class: "w-12 h-12 rounded-full bg-muted" }
                div { class: "flex-1",
                    div { class: "h-5 w-32 bg-muted rounded mb-2" }
