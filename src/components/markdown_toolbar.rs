...
pub fn MarkdownToolbar(props: MarkdownToolbarProps) -> Element {
    ...
    // Update disabled signal when prop changes
    use_effect(move || {
    ...
    rsx! {
        Toolbar {
            ...
            // Image button - either triggers upload dialog or inserts markdown
            ...
            // Nostr mention button (if callback provided)
...
pub fn apply_markdown_format(
    ...
    if format.is_line_prefix() {
        // For line prefixes (headings, lists, quotes), insert at line start
        ...
        // For wrapping formats (bold, italic, code, links)
        ...
        // Position cursor appropriately based on format type
        let new_cursor = match format {
            // For links and images, position cursor at the URL placeholder
            // so user can immediately type the URL
            MarkdownFormat::Link | MarkdownFormat::Image => {
                // Cursor goes to "url" part: prefix + selected + "](" = before the "url)"
                start + prefix.len() + selected.len() + 2 // +2 for "]("
            }
            _ => {
                // For other formats, put cursor after the inserted text
                start + prefix.len() + selected.len() + suffix.len()
            }
        };

        (new_content, new_cursor)
    } else {
        // For simple insertions (horizontal rule)
        let before = &content[..start];
        ...
        if let Some(document) = window.document() {
            if let Some(element) = document.get_element_by_id(textarea_id) {
                if let Ok(textarea) = element.dyn_into::<web_sys::HtmlTextAreaElement>() {
                    // Convert UTF-8 position to UTF-16 for DOM
        ...
        if let Some(document) = window.document() {
            if let Some(element) = document.get_element_by_id(textarea_id) {
                if let Ok(textarea) = element.dyn_into::<web_sys::HtmlTextAreaElement>() {
                    ...
                    // Convert UTF-16 positions to UTF-8
