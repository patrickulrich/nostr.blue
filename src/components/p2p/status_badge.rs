...
pub fn P2PTypeBadge(order_type: OrderType) -> Element {
    ...
    rsx! {
        span { class: "px-2 py-1 text-xs font-bold rounded {class}", "{label}" }
    }
}
...
pub fn P2PLayerBadge(layer: Layer) -> Element {
    ...
    rsx! {
        span { class: "inline-flex items-center gap-1 px-2 py-0.5 text-xs bg-muted rounded",
            span { "{icon}" }
            span { "{label}" }
        }
    }
...
pub fn P2PNetworkBadge(network: Network) -> Element {
    ...
    rsx! {
        span { class: "px-2 py-0.5 text-xs font-medium rounded {class}", "{label}" }
    }
}
