...
fn FilterChip(label: String, is_active: bool, on_toggle: EventHandler<()>) -> Element {
    rsx! {
        button {
            class: if is_active { "px-3 py-1 text-sm rounded-full bg-primary text-primary-foreground" } else { "px-3 py-1 text-sm rounded-full bg-muted hover:bg-accent transition" },
...
pub fn P2POrderFilters(
    ...
    rsx! {
        div { class: "p-4 bg-card border-b border-border space-y-4",
            div {
                h4 { class: "text-sm font-medium mb-2", "Status" }
            ...
            div {
                h4 { class: "text-sm font-medium mb-2", "Currency" }
                div { class: "flex flex-wrap gap-2 mb-2",
                    for curr in COMMON_CURRENCIES.iter() {
                        button {
                            class: if filters.read().currency.as_ref().map(|c| c == *curr).unwrap_or(false) { "px-3 py-1 text-sm rounded-full bg-primary text-primary-foreground" } else { "px-3 py-1 text-sm rounded-full bg-muted hover:bg-accent transition" },
                            onclick: {
            ...
            div {
                h4 { class: "text-sm font-medium mb-2", "Fiat Amount Range" }
            ...
            div {
                h4 { class: "text-sm font-medium mb-2", "Min Rating" }
                div { class: "flex flex-wrap gap-2",
                    for preset in &[0.0_f64, 3.0, 4.0, 4.5] {
                        {
                            let is_active = filters.read().min_rating == Some(*preset);
                            let label = if *preset == 0.0 { "Any".to_string() } else { format!("{preset}+") };
                            rsx! {
                                button {
                                    class: if is_active { "px-3 py-1 text-sm rounded-full bg-primary text-primary-foreground" } else { "px-3 py-1 text-sm rounded-full bg-muted hover:bg-accent transition" },
                                    onclick: move |_| {
                                        filters.write().min_rating = if *preset == 0.0 { None } else { Some(*preset) };
                                    },
                                    "{label}"
                                }
                            }
            ...
            div {
                h4 { class: "text-sm font-medium mb-2", "Network" }
                div { class: "flex flex-wrap gap-2",
                    NetworkFilterChip {
            ...
            div {
                h4 { class: "text-sm font-medium mb-2", "Layer" }
                div { class: "flex flex-wrap gap-2",
                    LayerFilterChip {
                    ...
                    LayerFilterChip { layer: Layer::Liquid, filters, label: "Liquid" }
                }
            }
            div {
                h4 { class: "text-sm font-medium mb-2", "Payment Methods" }
                div { class: "flex flex-wrap gap-2",
                    for method in COMMON_PAYMENT_METHODS.iter() {
                        PaymentMethodChip { method: method.to_string(), filters }
                    }
                }
            }
            div {
                h4 { class: "text-sm font-medium mb-2", "Platform" }
                div { class: "flex flex-wrap gap-2",
                    for platform in KNOWN_PLATFORMS.iter() {
                        button {
                            class: if filters.read().platform.as_ref().map(|p| p == *platform).unwrap_or(false) { "px-3 py-1 text-sm rounded-full bg-primary text-primary-foreground" } else { "px-3 py-1 text-sm rounded-full bg-muted hover:bg-accent transition" },
                            onclick: {
