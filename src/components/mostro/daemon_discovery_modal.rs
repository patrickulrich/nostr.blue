                            ...
                            for daemon in discovered_daemons.read().iter() {
                                {
                                    ...
                                    // C4: try kind-0 profile metadata first
                                    // (enqueued for fetch in discovery.rs), then
                                    // fall back to community label, LND alias,
                                    // or truncated pubkey. Mirrors Mobile's
                                    // `MostroNodeAvatar` + name enrichment.
                                    let profile_name = crate::stores::profiles::get_profile(&daemon.pubkey)
                                        .and_then(|m| m.name)
                                        .filter(|n| !n.trim().is_empty());
                                    let display_name = profile_name
                                        .or_else(|| daemon.community_label.map(|s| s.to_string()))
                                        .or_else(|| daemon.info.lnd_node_alias.clone())
                                        .unwrap_or_else(|| truncate_pubkey(&daemon.pubkey));
                                    let profile_picture = crate::stores::profiles::get_profile(&daemon.pubkey)
                                        .and_then(|m| m.picture)
                                        ...
                                        div {
                                            ...
                                            div { class: "flex items-start justify-between gap-2",
                                                div { class: "flex items-start gap-3 flex-1 min-w-0",
                                                    // C4: avatar from kind-0 metadata (https-only,
                                                    // per Mobile's URL-sanitization pattern).
                                                    if let Some(ref url) = profile_picture {
                                                        img {
                                                            src: "{url}",
                                                            alt: "{display_name}",
                                                            class: "w-10 h-10 rounded-full object-cover shrink-0",
                                                            // Don't let a broken image break the layout.
                                                            onerror: move |_| {},
                                                        }
                                                    } else {
                                                        div { class: "w-10 h-10 rounded-full bg-muted flex items-center justify-center text-lg shrink-0",
                                                            "🧌"
                                                        }
                                                    }
