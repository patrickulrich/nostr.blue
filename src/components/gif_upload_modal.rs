        ...
        div {
            ...
            div {
                ...
                div { class: "p-4 space-y-4",
                    if *success.read() {
                        div { class: "p-4 bg-green-100 dark:bg-green-900/30 border border-green-500/30 rounded-lg text-green-700 dark:text-green-300 text-center",
                            span { class: "text-2xl", "✓" }
                            p { class: "font-medium mt-2", "GIF uploaded successfully!" }
                            p { class: "text-sm opacity-75",
                                "Your GIF is now available in the GIF picker."
                            }
                        }
                    }
                    if !*success.read() {
                            ...
                            if let Some((filename, data, _, preview_url)) = selected_file.read().as_ref() {
                                div { class: "p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl space-y-3",
                                    div { class: "flex items-start gap-4",
                                        div { class: "w-24 h-24 rounded-lg overflow-hidden bg-gray-200 dark:bg-gray-600 shrink-0",
                        ...
                        if let Some(err) = error.read().as_ref() {
                            div { class: "p-3 bg-red-100 dark:bg-red-900/30 border border-red-500/30 rounded-lg text-red-700 dark:text-red-300 text-sm",
                                "{err}"
                            }
                        }
                    }
