    ...
    rsx! {
        div { class: "flex gap-4 p-4 bg-card border border-border rounded-lg",
            ...
            div { class: "flex-1 min-w-0",
                h3 { class: "font-medium line-clamp-2 mb-1", "{product.title}" }
                div { class: "text-sm text-muted-foreground mb-2",
                    ...
                    if let Some(stock) = product.stock {
                        if stock == 0 {
                            ...
                            span { class: "ml-2 text-yellow-600 dark:text-yellow-500 font-medium",
                                "• Max available"
                            }
                        } else if stock <= 5 {
                            span { class: "ml-2 text-yellow-600 dark:text-yellow-500",
                                "• {stock} left"
                            }
                        }
                    }
