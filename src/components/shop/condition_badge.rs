...
pub fn ConditionBadge(props: ConditionBadgeProps) -> Element {
    ...
    let (bg_class, text_class, label): (&str, &str, String) = match condition.as_str() {
        ...
        "used" => ("bg-amber-100 dark:bg-amber-900/30", "text-amber-700 dark:text-amber-400", "Used".to_string()),
        "good" => ("bg-amber-100 dark:bg-amber-900/30", "text-amber-700 dark:text-amber-400", "Good".to_string()),
