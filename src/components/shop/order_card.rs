        ...
        div { class: "bg-card border border-border rounded-lg p-4 hover:border-ring transition",
            div { class: "flex items-start justify-between mb-3",
                ...
                div { class: "text-right",
                    div { class: "font-semibold text-amber-500", "⚡{total_formatted} sats" }
            ...
            }
            {
                ...
                rsx! {
                    p { class: "text-sm text-muted-foreground", "{counterparty}" }
                }
            }
            if let Some(shipping) = &order.shipping_status {
                div { class: "mt-2 text-xs text-muted-foreground", "Shipping: {shipping}" }
            }
        }
    }
}
