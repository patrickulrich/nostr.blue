                ...
                if image_count > 1 {
                    ...
                    div { class: "absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1.5",
                        for i in 0..image_count {
                            button {
                                key: "{i}",
                                class: if i == current { "w-2 h-2 rounded-full bg-white" } else { "w-2 h-2 rounded-full bg-white/50 hover:bg-white/75 transition" },
            ...
            if image_count > 1 {
                div { class: "flex gap-2 overflow-x-auto pb-2",
                    for (i , img) in images.iter().enumerate() {
                        button {
                            key: "{i}",
                            class: if i == current { "w-16 h-16 shrink-0 rounded-lg overflow-hidden ring-2 ring-blue-500" } else { "w-16 h-16 shrink-0 rounded-lg overflow-hidden opacity-70 hover:opacity-100 transition" },
                            onclick: move |_| current_index.set(i),
                            img {
                                src: "{img}",
