        ...
        div {
            ...
            button {
                ...
                div { class: "flex-1 min-w-0",
                    ...
                    div { class: "text-xs text-muted-foreground mt-1", "{proof_count} proofs" }
                }
                div { class: "flex items-center gap-3",
                    div { class: "text-right",
                        div { class: "font-bold", "{format_sats_with_separator(total_balance)} sats" }
                    }
                ...
                div { class: "border-t border-border",
                    if tokens_for_mint.is_empty() {
                        div { class: "px-4 py-6 text-center",
                            p { class: "text-muted-foreground text-sm", "No tokens for this mint" }
                    ...
                    for (i , token) in tokens_for_mint.iter().enumerate() {
                        div {
                            ...
                            div { class: "flex items-start justify-between mb-2",
                                div { class: "text-xs text-muted-foreground", "Token Event" }
                                ...
                                for (proof_idx , proof) in token.proofs.iter().enumerate() {
                                    div {
                                        ...
                                        div { class: "flex justify-between items-center",
                                            ...
                                            div { class: "font-bold", "{proof.amount} sats" }
                                        }
                    ...
                    if *show_mint_info.read() {
                        div { class: "px-4 py-3 border-t border-border bg-blue-50 dark:bg-blue-950/20",
                                ...
                                div { class: "space-y-2",
                                    ...
                                    if let Some(desc_long) = &info.description_long {
                                        if info.description.as_ref() != Some(desc_long) {
                                            div { class: "text-xs text-muted-foreground mt-1 bg-background/30 rounded p-2",
                                                "{desc_long}"
                                            }
                                        }
                                    }
                                    ...
                                    if let Some(motd) = &info.motd {
                                        div { class: "text-xs italic bg-background/50 rounded p-2 mt-2",
                                            "{motd}"
                                        }
                                    }
                    ...
                    div { class: "px-4 py-3 border-t border-border bg-background/30 flex gap-2",
                        button {
                            class: if *mint_info_loading.read() { "px-3 py-2 text-sm bg-blue-500 text-white rounded-lg opacity-50 cursor-not-allowed" } else if *show_mint_info.read() { "px-3 py-2 text-sm bg-blue-600 text-white rounded-lg" } else { "px-3 py-2 text-sm bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition" },
                        ...
                        div { class: "flex-1",
                            button {
                                class: if *is_cleaning.read() { "w-full px-3 py-2 text-sm bg-accent rounded-lg opacity-50 cursor-not-allowed" } else { "w-full px-3 py-2 text-sm bg-accent hover:bg-accent/80 rounded-lg transition" },
                                        ...
                                        spawn(async move {
                                            match cashu::cleanup_spent_proofs(mint_url).await {
                                                Ok((count, amount)) if count > 0 => {
                                                    cleanup_message
                                                        .set(
                                                            Some(format!("Cleaned {} proofs ({} sats)", count, amount)),
                                                        );
                            ...
                            if let Some(msg) = cleanup_message.read().as_ref() {
                                div { class: "mt-1 text-xs text-center text-muted-foreground",
                                    "{msg}"
                                }
                            }
                        }
                            ...
                            if *show_confirm.read() {
                                div { class: "flex flex-col gap-2",
                                    ...
                                    if let Some(error) = remove_error.read().as_ref() {
                                        div { class: "text-xs text-destructive text-center bg-destructive/10 rounded p-2",
                                            "Failed: {error}"
                                        }
                                    }
    ...
    if !has_tokens && !has_mints {
        return rsx! {
            div { class: "bg-card border border-border rounded-lg p-8 text-center",
                ...
                p { class: "text-muted-foreground", "No tokens yet" }
                p { class: "text-sm text-muted-foreground mt-1", "Receive tokens to get started" }
            }
        };
    }
