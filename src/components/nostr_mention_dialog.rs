...
pub fn NostrMentionDialog(props: NostrMentionDialogProps) -> Element {
    ...
    rsx! {
        Modal { open,
            div { class: "w-full max-w-lg max-h-[70vh] flex flex-col",
                ModalHeader {
...
fn TabButton(props: TabButtonProps) -> Element {
    ...
    rsx! {
        button { class, onclick: props.onclick, {props.children} }
    }
}
#[derive(Props, Clone, PartialEq)]
    ...
    rsx! {
        button {
            ...
            div { class: "flex-1 min-w-0",
                p { class: "font-medium truncate", "{display_name}" }
                div { class: "flex items-center gap-2 text-xs text-muted-foreground",
                    if let Some(name) = username {
                        span { "@{name}" }
                    }
                    span { class: "truncate", "{npub_short}" }
                }
            }
        ...
        button {
            ...
            div { class: "flex items-center gap-2 text-xs text-muted-foreground",
                ...
                if props.result.is_from_contact {
                    span { class: "ml-auto px-2 py-0.5 rounded-full bg-primary/10 text-primary",
                        "Contact"
                    }
                }
            }
            p { class: "text-sm line-clamp-2", "{content_preview}" }
        }
    }
}
#[derive(Props, Clone, PartialEq)]
        ...
        button {
            ...
            div { class: "flex-1 min-w-0",
                ...
                if let Some(summary_text) = summary {
                    p { class: "text-sm text-muted-foreground line-clamp-2", "{summary_text}" }
                }
