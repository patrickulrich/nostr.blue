...
pub fn ArtistCard(props: ArtistCardProps) -> Element {
    ...
    rsx! {
        Link {
            to: crate::routes::Route::MusicArtist {
                artist_id: result.id.clone(),
            },
            ...
            div { class: "flex-1 min-w-0",
                ...
                div { class: "text-xs text-muted-foreground", "Artist" }
            }
        }
    }
}
...
pub fn ArtistCardSkeleton() -> Element {
    rsx! {
        div { class: "flex items-center gap-3 p-3 rounded-lg animate-pulse",
            div { class: "w-14 h-14 bg-muted rounded-full shrink-0" }
