    ...
    rsx! {
        div { class: "max-w-2xl mx-auto p-4",
            div { class: "bg-card border border-border rounded-lg p-8 text-center",
                ...
                h2 { class: "text-2xl font-bold mb-4", "Create Cashu Wallet" }
                ...
                if *creating.read() {
                    ...
                    p { class: "text-muted-foreground", "Creating wallet..." }
                } else if let Some(err) = error_msg.read().clone() {
                    div { class: "p-3 bg-destructive/10 border border-destructive rounded-lg text-destructive text-sm mb-4",
                        {err}
                    }
                    button {
