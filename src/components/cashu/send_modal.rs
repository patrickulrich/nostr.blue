        ...
        div {
            ...
            div {
                ...
                div { class: "px-6 py-4 border-b border-border flex items-center justify-between",
                    h3 { class: "text-xl font-bold", "Send Tokens" }
                    button {
                ...
                div { class: "p-6 space-y-4",
                    ...
                    div { class: "flex items-center justify-between py-2",
                        div {
                            label { class: "text-sm font-semibold", "Lock to recipient (P2PK)" }
                            p { class: "text-xs text-muted-foreground",
                                "Only the specified npub can redeem this token"
                            }
                        }
                        button {
                            class: if *p2pk_enabled.read() { "w-12 h-6 rounded-full bg-blue-500 relative transition-colors" } else { "w-12 h-6 rounded-full bg-gray-300 dark:bg-gray-600 relative transition-colors" },
                            onclick: move |_| {
                    ...
                    if let Some(token) = token_result.read().as_ref() {
                        div { class: "bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 rounded-lg p-4",
                            div { class: "space-y-2",
                                    ...
                                    div { class: "flex-1",
                                        div { class: "flex items-center justify-between",
                                            ...
                                            match *token_claimed.read() {
                                                Some(true) => rsx! {
                                                    span { class: "px-2 py-0.5 text-xs font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded-full",
                                                        "Claimed"
                                                    }
                                                },
                                                Some(false) => rsx! {
                                                    span { class: "px-2 py-0.5 text-xs font-medium bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded-full animate-pulse",
                                                        "Pending"
                                                    }
                                                },
                                                None => rsx! {},
                                ...
                                div {
                                    label { class: "block text-xs font-semibold mb-1 text-green-800 dark:text-green-200",
                                        "Share this token:"
                                    }
                                                    ...
                                                    if let Some(window) = web_sys::window() {
                                                        if let Some(document) = window.document() {
                                                            if let Some(textarea) = document
                                                                .query_selector("#send-token")
                                                                .ok()
                                                                .flatten()
                                                            {
                                                                if let Ok(element) = textarea
                    ...
                    div { class: "bg-accent/50 rounded-lg p-4",
                        h4 { class: "text-sm font-semibold mb-2", "Preview" }
