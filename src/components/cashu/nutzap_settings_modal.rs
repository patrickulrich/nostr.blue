...
pub fn NutzapSettingsModal(on_close: EventHandler<()>) -> Element {
    ...
    let on_publish = move |_| {
        ...
        let mints: Vec<cashu::NutzapMint> = selected_mints
            .read()
            .iter()
            .map(|url| cashu::normalize_mint_url(url))
            .filter(|normalized_url| seen_mints.insert(normalized_url.clone()))
    ...
    rsx! {
        div {
            ...
            div {
                ...
                div { class: "px-6 py-4 border-b border-border flex items-center justify-between sticky top-0 bg-card",
                    h3 { class: "text-xl font-bold", "Nutzap Settings" }
                    button {
                ...
                div { class: "p-6 space-y-6",
                    div { class: "flex items-center justify-between p-4 bg-accent/50 rounded-lg",
                        ...
                        div { class: if nutzap_enabled { "px-3 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-full text-sm font-medium" } else { "px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded-full text-sm font-medium" },
                            if nutzap_enabled {
                                "Enabled"
                            } else {
                                "Disabled"
                    ...
                    if let Some(pubkey) = &p2pk_pubkey {
                        div {
                            label { class: "block text-sm font-semibold mb-2", "Your P2PK Pubkey" }
                            div { class: "px-4 py-2 bg-background border border-border rounded-lg font-mono text-xs break-all",
                                "{pubkey}"
                            }
                    ...
                    div {
                        label { class: "block text-sm font-semibold mb-2", "Accepted Mints" }
                                ...
                                for mint_url in all_mints.iter() {
                                    {
                                        ...
                                        rsx! {
                                            button {
                                                key: "mint-{mint_url}",
                                                class: if is_selected { "w-full px-4 py-3 text-left rounded-lg border-2 border-blue-500 bg-blue-50 dark:bg-blue-900/20 transition" } else { "w-full px-4 py-3 text-left rounded-lg border border-border hover:border-blue-300 bg-background transition" },
                                                onclick: move |_| toggle_mint(mint_url_clone.clone()),
                                                div { class: "flex items-center gap-3",
                                                    div { class: if is_selected { "w-5 h-5 rounded border-2 border-blue-500 bg-blue-500 flex items-center justify-center text-white text-xs" } else { "w-5 h-5 rounded border-2 border-gray-300 dark:border-gray-600" },
                                                        if is_selected {
                                                            "v"
                                                        }
                                                    }
                                                    span { class: "font-mono text-sm truncate", {shorten_url(mint_url, 40)} }
                                                }
                                            }
                                        }
                                    }
                    ...
                    div {
                        label { class: "block text-sm font-semibold mb-2", "Delivery Relays" }
                        p { class: "text-xs text-muted-foreground mb-3",
                    ...
                    div { class: "flex items-center justify-between py-3",
                        div {
                            label { class: "text-sm font-semibold", "Auto-redeem nutzaps" }
                        ...
                        button {
                            ...
                            class: if *auto_redeem_enabled.read() { "w-12 h-6 rounded-full bg-blue-500 relative transition-colors" } else { "w-12 h-6 rounded-full bg-gray-300 dark:bg-gray-600 relative transition-colors" },
                            onclick: move |_| {
                                let current = *auto_redeem_enabled.read();
                                auto_redeem_enabled.set(!current);
                            },
                            div { class: if *auto_redeem_enabled.read() { "w-5 h-5 rounded-full bg-white absolute top-0.5 right-0.5 transition-all" } else { "w-5 h-5 rounded-full bg-white absolute top-0.5 left-0.5 transition-all" } }
                        }
                    }
                ...
                div { class: "px-6 py-4 border-t border-border flex gap-3 sticky bottom-0 bg-card",
                    button {
                    ...
                    }
                    {
                        ...
                        rsx! {
                            button {
                                class: if is_disabled { "flex-1 px-4 py-3 bg-blue-500 text-white font-semibold rounded-lg transition opacity-50 cursor-not-allowed" } else { "flex-1 px-4 py-3 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-lg transition" },
