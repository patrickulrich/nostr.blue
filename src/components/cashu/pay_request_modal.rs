        ...
        div {
            ...
            div {
                ...
                div { class: "flex items-center justify-between mb-6",
                    h2 { class: "text-xl font-bold", "Pay Request" }
                    if !matches!(*pay_state.read(), PayState::Paying) {
                        button {
                ...
                match &*pay_state.read() {
                    PayState::Input => rsx! {
                        div {
                            div { class: "mb-4",
                                label { class: "block text-sm font-medium mb-2", "Payment Request" }
                                textarea {
                            ...
                            p { class: "text-xs text-muted-foreground mt-3 text-center", "Your balance: {balance()} sats" }
                        }
                    },
                    PayState::Parsed { request } => {
                        ...
                        let has_nostr = request
                            .transports
                            .iter()
                            .any(|t| t._type == TransportType::Nostr);
                        rsx! {
                            div {
                                div { class: "bg-background border border-border rounded-lg p-4 mb-4",
                                    div { class: "mb-3",
                                        div { class: "text-sm text-muted-foreground", "Amount" }
                                    ...
                                    if has_description {
                                        div { class: "mb-3",
                                            ...
                                            div { class: "text-foreground", "{description}" }
                                        }
                                    }
                                    if mint_count > 0 {
                                        div { class: "mb-3",
                                            div { class: "text-sm text-muted-foreground", "Accepted Mints" }
                                            div { class: "text-sm",
                                    ...
                                    div {
                                        div { class: "text-sm text-muted-foreground", "Delivery" }
                                        div { class: "text-sm flex items-center gap-2",
                                            if has_nostr {
                                                span { class: "text-purple-500", "Via Nostr (encrypted)" }
                                            } else {
                                                span { class: "text-muted-foreground", "Manual / HTTP" }
                                            }
                                        }
                                    }
                                }
                                if !amount_specified {
                                    div { class: "mb-4",
                                        label { class: "block text-sm font-medium mb-2", "Amount to Send" }
                                        div { class: "relative",
                                            input {
                                ...
                                }
                                {
                                    ...
                                    rsx! {
                                        if insufficient_balance {
                                            div { class: "mb-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg text-yellow-600 dark:text-yellow-400 text-sm",
                    ...
                    PayState::Paying => rsx! {
                        div { class: "text-center py-8",
                            p { class: "text-muted-foreground", "Sending payment..." }
                        }
                    },
                    PayState::Success { amount } => rsx! {
                        div { class: "text-center py-8",
                            div { class: "text-6xl mb-4", "✅" }
                            h3 { class: "text-xl font-bold text-green-500 mb-2", "Payment Sent!" }
                            p { class: "text-lg", "{amount} sats" }
                            button {
                    ...
                    PayState::Error { message } => rsx! {
                        div { class: "text-center py-8",
                            div { class: "text-6xl mb-4", "❌" }
                            h3 { class: "text-xl font-bold text-red-500 mb-2", "Error" }
