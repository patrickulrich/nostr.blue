        ...
        div {
            ...
            div {
                ...
                div { class: "p-6 space-y-4",
                    ...
                    if !*is_confirmed.read() {
                        div {
                            label { class: "block text-sm font-semibold mb-2", "Mint URL" }
                            input {
                    ...
                    if let Some(info) = mint_info.read().as_ref() {
                        div { class: "bg-accent/50 rounded-lg p-4 space-y-3",
                            h4 { class: "text-sm font-semibold mb-2", "Mint Information" }
                            ...
                            div { class: "flex gap-4 mt-2",
                                div { class: if info.supported_nuts.contains(&4) { "text-green-500 text-sm" } else { "text-red-500 text-sm" },
                                    if info.supported_nuts.contains(&4) {
                                        "✓ NUT-4 (Mint)"
                                    } else {
                                        "✗ NUT-4 (Mint)"
                                    }
                                }
                                div { class: if info.supported_nuts.contains(&5) { "text-green-500 text-sm" } else { "text-red-500 text-sm" },
                                    if info.supported_nuts.contains(&5) {
                                        "✓ NUT-5 (Melt)"
                                    } else {
                                        "✗ NUT-5 (Melt)"
                                    }
                            ...
                            if let Some(motd) = &info.motd {
                                div { class: "mt-2 p-2 bg-background/50 rounded text-sm italic",
                                    "{motd}"
                                }
                            }
                ...
                div { class: "px-6 py-4 border-t border-border flex gap-3",
                    if *is_confirmed.read() {
                        ...
                        if mint_info.read().is_some() {
                            button {
                                class: if *is_adding.read() { "flex-1 px-4 py-3 bg-green-500 text-white font-semibold rounded-lg transition opacity-50 cursor-not-allowed" } else { "flex-1 px-4 py-3 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-lg transition" },
                            ...
                            button {
                                class: if *is_checking.read() || mint_url.read().is_empty() { "flex-1 px-4 py-3 bg-blue-500 text-white font-semibold rounded-lg transition opacity-50 cursor-not-allowed" } else { "flex-1 px-4 py-3 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-lg transition" },
