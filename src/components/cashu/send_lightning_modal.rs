            ...
            div {
                ...
                div { class: "p-6 space-y-4",
                    if let Some((paid, preimage, fee)) = payment_result.read().as_ref() {
                        if *paid {
                            div { class: "bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 rounded-lg p-4 space-y-2",
                                div { class: "flex items-start gap-3",
                                    ...
                                    div {
                                        ...
                                        if let Some(pre) = preimage {
                                            p { class: "text-xs text-green-700 dark:text-green-300 mt-1 font-mono break-all",
                                                "Preimage: {pre}"
                                            }
                                        }
                            ...
                            div { class: "bg-yellow-50 dark:bg-yellow-950/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4 space-y-2",
                                div { class: "flex items-start gap-3",
                                    div { class: "text-2xl", "..." }
                                    div {
                        ...
                        div { class: "bg-accent/50 rounded-lg p-4 space-y-2",
                            ...
                            div { class: "space-y-2 text-sm",
                                for contrib in &q.contributions {
                                    div { class: "flex justify-between text-xs",
                                        span { class: "text-muted-foreground truncate max-w-[150px]",
                                            "{shorten_url(&contrib.mint_url, 30)}"
                                        }
                                        span { class: "font-mono",
                                            "{contrib.amount} + {contrib.fee_reserve} sats"
                                        }
                                    }
                                }
                                div { class: "flex justify-between border-t border-border pt-2",
                                ...
                                div { class: "flex justify-between border-t border-border pt-2",
                                    ...
                                    span { class: "font-mono font-semibold",
                                        "{q.total_amount + q.total_fee_reserve} sats"
                                    }
                                }
                            }
                        }
                    ...
                    if *is_paying.read() {
                        div { class: "bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4",
                            h4 { class: "text-sm font-semibold mb-3 text-blue-800 dark:text-blue-200",
                                "Payment Progress"
                            }
                            div { class: "space-y-2",
                                ProgressStep {
                                    label: "Preparing payment",
                                    is_active: matches!(*melt_progress, Some(MeltProgress::PreparingPayment)),
                                    is_complete: matches!(
                                        *melt_progress,
                                        Some(MeltProgress::PayingInvoice)
                                        | Some(MeltProgress::WaitingForConfirmation)
                                        | Some(MeltProgress::Completed { .. })
                                    ),
                                }
                                ProgressStep {
                                    label: "Sending to Lightning Network",
                                    is_active: matches!(*melt_progress, Some(MeltProgress::PayingInvoice)),
                                    is_complete: matches!(
                                        *melt_progress,
                                        Some(MeltProgress::WaitingForConfirmation) | Some(MeltProgress::Completed { .. })
                                    ),
                                }
                                ProgressStep {
                                    label: "Waiting for confirmation",
                                    is_active: matches!(*melt_progress, Some(MeltProgress::WaitingForConfirmation)),
                                    is_complete: matches!(*melt_progress, Some(MeltProgress::Completed { .. })),
                                }
                            }
                            div { class: "flex items-center justify-center text-sm text-blue-700 dark:text-blue-300 mt-4",
                    ...
                    if quote_info.read().is_none() && mpp_quote.read().is_none()
                        && payment_result.read().is_none()
                    {
                        div {
                        ...
                        if mints.len() > 1 {
                            div { class: "flex items-center gap-3 p-3 bg-accent/30 rounded-lg",
                                input {
                                    r#type: "checkbox",
                                    id: "mpp-mode",
                        ...
                        if *payment_mode.read() == PaymentMode::Mpp {
                            div { class: "space-y-2",
                                ...
                                if mpp_mint_balances.read().is_empty() {
                                    div { class: "bg-yellow-50 dark:bg-yellow-950/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3",
                        ...
                        div { class: "text-sm text-muted-foreground",
                            "Total available: "
                            span { class: "font-mono font-semibold",
                                "{WALLET_BALANCES.read().available} sats"
                            }
                        }
                    }
                }
                div { class: "px-6 py-4 border-t border-border flex gap-3",
                    button {
                        ...
                        if payment_result.read().is_some() {
                            "Close"
                        } else {
                            "Cancel"
                        }
                    ...
                    if payment_result.read().is_none() {
                        if quote_info.read().is_some() || mpp_quote.read().is_some() {
                            button {
                                class: if *is_paying.read() { "flex-1 px-4 py-3 bg-orange-500 text-white font-semibold rounded-lg transition opacity-50 cursor-not-allowed" } else { "flex-1 px-4 py-3 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-lg transition" },
                            ...
                            button {
                                class: if *is_creating_quote.read() || invoice.read().is_empty() { "flex-1 px-4 py-3 bg-blue-500 text-white font-semibold rounded-lg transition opacity-50 cursor-not-allowed" } else { "flex-1 px-4 py-3 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-lg transition" },
