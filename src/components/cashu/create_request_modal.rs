        ...
        div {
            ...
            div {
                ...
                div { class: "flex items-center justify-between mb-6",
                    h2 { class: "text-xl font-bold", "Create Payment Request" }
                    button {
                ...
                if payment_received {
                    ...
                    div {
                        div { class: "mb-4",
                            ...
                            div { class: "bg-background border border-border rounded-lg p-3 break-all text-xs font-mono",
                                {request_string.read().as_ref().unwrap_or(&String::new()).clone()}
                            }
                        }
                        button {
                            class: if copy_error.read().is_some() { "w-full py-3 bg-red-500/20 border border-red-500/50 text-red-500 rounded-lg font-semibold transition flex items-center justify-center gap-2 mb-4" } else { "w-full py-3 bg-accent hover:bg-accent/80 rounded-lg font-semibold transition flex items-center justify-center gap-2 mb-4" },
                            onclick: handle_copy,
                            ...
                            div { class: "text-center text-sm text-muted-foreground",
                                match &*progress {
                                    ...
                                    Some(PaymentRequestProgress::Error { message }) => rsx! {
                                        div { class: "text-red-500", "Error: {message}" }
                                    },
                                    _ => rsx! {
                    ...
                    div {
                        div { class: "mb-4",
                            label { class: "block text-sm font-medium mb-2", "Amount (optional)" }
                            div { class: "relative",
                                input {
                        ...
                        div { class: "mb-6",
                            label { class: "block text-sm font-medium mb-2", "Receive via" }
                            div { class: "flex gap-2",
                                button {
                                    class: if *use_nostr_transport.read() { "flex-1 py-2 px-4 rounded-lg font-medium transition bg-purple-500 text-white" } else { "flex-1 py-2 px-4 rounded-lg font-medium transition bg-background border border-border hover:bg-accent" },
                        ...
                        if let Some(err) = error_message.read().as_ref() {
                            div { class: "mb-4 p-3 bg-red-500/10 border border-red-500/30 rounded-lg text-red-500 text-sm",
                                "{err}"
                            }
                        }
                        button {
