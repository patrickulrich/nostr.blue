...
pub fn CashuReceiveLightningModal(on_close: EventHandler<()>) -> Element {
    ...
    let mut quote_info = use_signal(|| Option::<cashu::MintQuoteInfo>::None);
    ...
    let on_generate = move |_| {
        ...
        spawn(async move {
            match cashu::create_mint_quote(mint, amount_sats, None).await {
                Ok(quote) => {
            ...
            div {
                ...
                div { class: "p-6 space-y-4",
                    ...
                    if let Some(q) = quote_info.read().as_ref() {
                        div { class: "space-y-4",
                            ...
                            div {
                                label { class: "block text-xs font-semibold mb-1", "Lightning Invoice:" }
                                                ...
                                                if let Some(window) = web_sys::window() {
                                                    if let Some(document) = window.document() {
                                                        if let Some(textarea) = document
                                                            .query_selector("#lightning-invoice")
                                                            .ok()
                                                            .flatten()
                                                        {
                            ...
                            if *is_polling.read() {
                                div { class: "flex items-center justify-center text-sm text-muted-foreground",
                                    if let Some(status) = mint_status.read().as_ref() {
                ...
                div { class: "px-6 py-4 border-t border-border flex gap-3",
                    ...
                    if quote_info.read().is_none() {
                        button {
                            class: if *is_generating.read() || amount.read().is_empty() { "flex-1 px-4 py-3 bg-blue-500 text-white font-semibold rounded-lg transition opacity-50 cursor-not-allowed" } else { "flex-1 px-4 py-3 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-lg transition" },
