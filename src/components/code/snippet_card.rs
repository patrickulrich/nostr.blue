...
pub fn CodeSnippetCard(
    ...
    rsx! {
        Link {
            to: Route::CodeSnippetDetail { note_id: snippet.event_id.clone() },
            class: "block border border-border rounded-lg overflow-hidden hover:border-accent transition",

            // Header
            div {
                class: "flex items-center justify-between px-4 py-2 bg-muted/50 border-b border-border",
                ...
                // Language badge
                span {
                    class: "px-2 py-0.5 text-xs font-medium rounded bg-accent text-accent-foreground",
                    "{language}"
                }
            }
            ...
            div {
                ...
                // Fade out gradient if truncated
                if truncated {
                    div {
            ...
            // Footer with description and author
            if !description.is_empty() || show_author {
                div {
                    class: "px-4 py-2 border-t border-border bg-muted/30",

                    if !description.is_empty() {
                        p {
                            class: "text-sm text-muted-foreground line-clamp-2 mb-1",
                            "{description}"
                        }
                    }
...
pub fn CodeSnippetEmbed(snippet: DisplaySnippet) -> Element {
    ...
    rsx! {
        div {
            class: "my-2 border border-border rounded-lg overflow-hidden",

            // Header
            div {
                class: "flex items-center justify-between px-3 py-1.5 bg-muted/50 border-b border-border",
                ...
                div {
                    ...
                    span {
                        class: "px-1.5 py-0.5 text-xs font-medium rounded bg-accent text-accent-foreground",
                        "{language}"
                    }
                }
...
pub fn CodeSnippetRef(event_id: String) -> Element {
    rsx! {
        Link {
            to: Route::CodeSnippetDetail { note_id: event_id.clone() },
