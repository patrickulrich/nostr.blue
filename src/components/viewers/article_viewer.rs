...
#[component]
pub fn ArticleViewer(naddr: String) -> Element {
            ...
            div { class: "max-w-4xl mx-auto px-4 py-8",
                ...
                if let Some(err) = error.read().as_ref() {
                    div { class: "text-center py-12",
                        ...
                        h3 { class: "text-xl font-semibold mb-2", "Error Loading Article" }
                ...
                if let Some(event) = article.read().as_ref().cloned() {
                    {
                        ...
                        rsx! {
                            article { class: "space-y-6",
                                h1 { class: "text-4xl md:text-5xl font-bold leading-tight", "{title}" }
                                if let Some(sum) = summary {
                                    p { class: "text-xl text-muted-foreground leading-relaxed", "{sum}" }
                                }
                                    ...
                                    Link {
                                        ...
                                        div { class: "w-12 h-12 rounded-full overflow-hidden bg-muted flex items-center justify-center",
                                            if let Some(pic_url) = profile_picture {
                                                ...
                                                span { class: "text-lg font-semibold text-muted-foreground", "{avatar_letter}" }
                                            }
                                        }
                                        div {
                                            div { class: "font-semibold", "{display_name}" }
                                            div { class: "text-sm text-muted-foreground", "{timestamp} · {read_time} min read" }
                                        }
                                    }
                                }
                                ...
                                if let Some(img_url) = image_url {
                                    div { class: "rounded-lg overflow-hidden",
                                        img { src: "{img_url}", alt: "{title}", class: "w-full h-auto" }
                                    }
                                }
                                ArticleContent { content: event.content.clone() }
                                    ...
                                    div { class: "flex items-center justify-center gap-4",
                                        {
                                            ...
                                            rsx! {
                                                button {
                                                    class: if *is_liked.read() { "flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500/10 text-red-500 transition" } else { "flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-accent transition" },
                                                    ...
                                                    HeartIcon { class: "w-5 h-5", filled: *is_liked.read() }
                                                    span {
                                                        if *is_liking.read() {
                                                            "..."
