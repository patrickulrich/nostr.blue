...
#[component]
pub fn VideoViewer(video_id: String) -> Element {
    ...
    use_effect(use_reactive!(|video_id| {
        let (id, _) = parse_video_id_and_feed(&video_id);
    ...
    rsx! {
        div { class: "min-h-screen bg-black",
            if !*nostr_client::CLIENT_INITIALIZED.read()
                || (*loading.read() && video_event.read().is_none())
            {
                ClientInitializing {}
                ...
                div { class: "flex items-center justify-center h-screen text-white",
                    div { class: "text-center",
                        ...
                        p { class: "text-red-400 mb-4", "Error loading video: {err}" }
                        Link {
                ...
                div { class: "flex items-center justify-center h-screen text-white",
                    div { class: "text-center",
                        ...
                        p { class: "text-gray-400 mb-4", "Video not found" }
                        Link {
                            to: crate::routes::Route::Videos {},
                            class: "text-blue-400 hover:underline",
                            "← Back to Videos"
        ...
        div { class: "min-h-screen bg-background",
            div { class: "sticky top-0 z-20 bg-black/80 backdrop-blur-sm border-b border-gray-800",
                div { class: "px-4 py-3 flex items-center gap-3",
                    ...
                    h2 { class: "text-xl font-bold text-white", "Video" }
                }
            }
            div { class: "max-w-[1400px] mx-auto p-4",
                ...
                div { class: "bg-card border border-border rounded-lg p-6 mb-4",
                    if let Some(title) = &video_meta.title {
                        h1 { class: "text-2xl font-bold mb-3", "{title}" }
                    }
                    AuthorInfo { pubkey: event.pubkey.to_string() }
                    if !event.content.is_empty() {
                        p { class: "text-muted-foreground mb-4 whitespace-pre-wrap",
                            "{event.content}"
                        }
                    }
                    div { class: "flex flex-wrap gap-4 text-sm text-muted-foreground mb-4",
                        ...
                        div { class: "flex items-center gap-2",
                            "📅"
                            span { {format_relative_time_or(event.created_at.as_secs(), "just now")} }
                        }
                    }
                    VideoInteractions {
                        event: event.clone(),
                ...
                div { class: "bg-card border border-border rounded-lg p-6",
                    div { class: "flex items-center justify-between mb-4",
                        h3 { class: "text-xl font-bold", "Comments" }
                        button {
...
pub(crate) fn VertsPlayer(
    ...
    rsx! {
        div {
            ...
            div { class: "absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-4 z-40",
                ...
                if !events.read().is_empty()
                    && (*current_video_index.read() < events.read().len() - 1 || *has_more.read())
                {
                    button {
                        class: "w-12 h-12 rounded-full bg-white/20 hover:bg-white/30 backdrop-blur-sm flex items-center justify-center text-white transition",
            ...
            div { class: "flex items-end justify-between",
                div { class: "flex-1 mr-4",
                    Link {
                        ...
                        span { class: "text-white font-semibold", "{display_name}" }
                    }
                    if let Some(title) = video_meta.title {
                        h3 { class: "text-white font-semibold text-lg mb-2", "{title}" }
                    }
                    if !event.content.is_empty() {
                        p { class: "text-white/90 text-sm line-clamp-2", "{event.content}" }
                    }
                ...
                div { class: "flex flex-col items-center gap-4",
                    button {
                        class: if *is_liked.read() { "text-red-500 hover:bg-white/20 p-3 rounded-full transition flex flex-col items-center" } else { "text-white hover:bg-white/20 p-3 rounded-full transition flex flex-col items-center" },
                        ...
                        if *like_count.read() > 0 {
                            span { class: if *is_liked.read() { "text-xs mt-1 font-semibold text-red-500" } else { "text-xs mt-1 font-semibold" },
                                {
                                    let count = *like_count.read();
                                    if count > 500 { "500+".to_string() } else { format_count(count) }
                                }
                            }
                        }
                    }
                    button {
                        ...
                        crate::components::icons::MessageCircleIcon { class: "w-6 h-6".to_string(), filled: false }
                    ...
                    }
                    {
                        ...
                        if has_lightning {
                            rsx! {
                                button { class: "text-white hover:bg-white/20 p-3 rounded-full transition flex flex-col items-center",
                                    crate::components::icons::ZapIcon { class: "w-6 h-6".to_string(), filled: false }
                                    if *zap_amount_sats.read() > 0 {
                                        span { class: "text-xs mt-1 font-semibold text-yellow-400",
                ...
                div {
                    ...
                    div {
                        ...
                        div { class: "sticky top-0 bg-card border-b border-border px-6 py-4 flex items-center justify-between z-10",
                            h3 { class: "text-lg font-semibold text-white", "Comments" }
                            button {
...
fn VideoInteractions(event: Event, is_muted: bool, on_mute_toggle: EventHandler<()>) -> Element {
    ...
    rsx! {
        div { class: "flex gap-3 pt-4 border-t border-border",
            button {
                class: if *is_liked.read() { "flex items-center gap-2 px-4 py-2 bg-red-500/20 text-red-500 hover:bg-red-500/30 rounded-lg transition" } else { "flex items-center gap-2 px-4 py-2 bg-accent hover:bg-accent/80 rounded-lg transition" },
                disabled: *is_liking.read(),
                onclick: move |_| {
                    if *is_liking.read() {
                        return;
                ...
                crate::components::icons::HeartIcon { class: "w-5 h-5".to_string(), filled: *is_liked.read() }
                span {
                    if *like_count.read() > 0 {
            ...
            button { class: "flex items-center gap-2 px-4 py-2 bg-accent hover:bg-accent/80 rounded-lg transition",
                crate::components::icons::ZapIcon { class: "w-5 h-5".to_string(), filled: false }
                span {
                    if *zap_amount_sats.read() > 0 {
...
fn AuthorInfo(pubkey: String) -> Element {
    ...
    rsx! {
        Link {
            ...
            div {
                span { class: "font-semibold", "{display_name}" }
            }
        }
    }
}
