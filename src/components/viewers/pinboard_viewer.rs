...
#[component]
pub fn PinboardViewer(naddr: String) -> Element {
                ...
                if *loading.read() {
                    ...
                    div { class: "mb-6",
                        if let Some(ref img_url) = b.image {
                            div { class: "relative w-full h-48 md:h-64 rounded-xl overflow-hidden mb-4",
                                img {
                                    src: "{img_url}",
                                    alt: "{b.title}",
                        ...
                        div { class: "flex flex-wrap items-start justify-between gap-4",
                            div {
                                h1 { class: "text-2xl font-bold mb-2", "{b.title}" }
                        ...
                        if let Some(ref desc) = b.description {
                            p { class: "mt-4 text-muted-foreground", "{desc}" }
                        }
                    }
                    if *pins_loading.read() {
                        PinMosaicGrid { pins: vec![], loading: true, skeleton_count: 8 }
