...
use dioxus::prelude::*;
use nostr_sdk::{
    Event as NostrEvent, EventId, Filter, Kind, Timestamp,
};
...
#[component]
pub fn PollViewer(noteid: String) -> Element {
    ...
    use_effect(use_reactive!(|noteid| {
        ...
        spawn(async move {
            loading.set(true);
            error.set(None);
            match nostr_client::parse_event_id(&noteid) {
                Some(parsed) => {
    ...
    {
        let event_id = poll_event.read().clone().map(|e| e.id);
        ...
        div { class: "min-h-screen",
            div { class: "sticky top-0 z-20 bg-background/80 backdrop-blur-sm border-b border-border",
                div { class: "px-4 py-3 flex items-center gap-4",
                    ...
                    h1 { class: "text-xl font-bold", "\u{1F4CA} Poll" }
                }
            }
            div { class: "max-w-2xl mx-auto",
                if !*nostr_client::CLIENT_INITIALIZED.read() {
                    ...
                    div { class: "text-center py-12 px-4",
                        div { class: "text-6xl mb-4", "\u{1F4CA}" }
