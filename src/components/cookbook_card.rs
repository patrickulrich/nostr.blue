    ...
    rsx! {
        Link {
            ...
            div { class: "relative h-full flex flex-col justify-end p-4 text-white",
                h3 { class: "text-lg font-bold mb-1 truncate", "{cookbook.title}" }
                if let Some(ref desc) = cookbook.description {
                    p { class: "text-sm text-white/90 truncate", "{desc}" }
                }
            }
        }
    }
...
pub fn CookbookCardSkeleton() -> Element {
    rsx! {
        div { class: "shrink-0 w-64 h-40 bg-muted rounded-xl animate-pulse" }
    }
}
