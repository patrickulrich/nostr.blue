...
pub fn ArticleContent(content: String) -> Element {
    // Defer markdown rendering to after initial paint for better TTI
    let html_resource = use_resource(move || {
    ...
    // Clone the result to avoid lifetime issues with the borrow
    let html_content = html_resource.read().clone();

    match html_content {
        Some(html_content) => rsx! {
            div {
        ...
        None => rsx! {
            // Skeleton loader while markdown renders
            div {
                class: "article-content animate-pulse space-y-4",
