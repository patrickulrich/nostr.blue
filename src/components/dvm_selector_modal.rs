        ...
        div {
            ...
            div {
                ...
                div { class: "px-4 py-3 border-b border-border flex items-center justify-between",
                    h3 { class: "text-lg font-semibold", "Select DVM Provider" }
                    button {
                ...
                div { class: "overflow-y-auto max-h-[60vh]",
                    button {
                        ...
                        div {
                            div { class: "font-medium", "Default DVM" }
                            div { class: "text-xs text-muted-foreground", "Trending content discovery" }
                        }
                        if selected.is_none() {
                            span { class: "text-green-500", "\u{2713}" }
                        }
                    }
                    if loading {
                                ...
                                rsx! {
                                    button {
                                        ...
                                        div { class: "flex-1 min-w-0",
                                            div { class: "font-medium truncate", "{provider.name}" }
                                            if let Some(about) = &provider.about {
                                                div { class: "text-xs text-muted-foreground line-clamp-1", "{about}" }
                                            }
                                        }
                                        if is_selected {
                                            span { class: "text-green-500 shrink-0", "\u{2713}" }
                                        }
                                    }
                                }
                            }
