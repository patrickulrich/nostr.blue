...
pub fn AlbumCard(props: AlbumCardProps) -> Element {
    ...
    rsx! {
        Link {
            to: crate::routes::Route::MusicAlbum {
                album_id: result.id.clone(),
            },
            ...
            div { class: "relative shrink-0",
                if let Some(ref art_url) = result.album_art_url {
                    ...
                    div { class: "w-14 h-14 rounded bg-muted flex items-center justify-center",
                        DiscIcon { class: "w-6 h-6 text-muted-foreground".to_string() }
                    }
                }
            }
            div { class: "flex-1 min-w-0",
                div { class: "font-medium text-sm truncate", "{result.name}" }
                if let Some(ref artist) = result.artist {
                    div { class: "text-xs text-muted-foreground truncate", "{artist}" }
                } else {
                    div { class: "text-xs text-muted-foreground", "Album" }
                }
            }
        }
    }
