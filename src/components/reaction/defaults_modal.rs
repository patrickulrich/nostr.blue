    ...
    rsx! {
        div {
            ...
            div {
                ...
                div { class: "flex flex-wrap gap-2 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg mb-4 min-h-[60px]",
                    for (index , reaction) in local_reactions.read().iter().cloned().enumerate() {
                        div {
                            ...
                            ondrop: move |e| {
                                ...
                                if let Some(from_str) = e.data_transfer().get_data("text/plain") {
                                    if let Ok(from_idx) = from_str.parse::<usize>() {
                                        local_reactions
                                            .with_mut(|list| {
                                                if from_idx != index && from_idx < list.len()
                                                    && index < list.len()
                                                {
                                                    let item = list.remove(from_idx);
                                                    let insert_idx = if from_idx < index {
                                                        index - 1
                                                    } else {
                                                        index
                                                    };
                            ...
                            div {
                                ...
                                if index == 0 {
                                    span { class: "absolute -top-2 left-1/2 -translate-x-1/2 text-[9px] bg-blue-500 text-white px-1.5 py-0.5 rounded-full whitespace-nowrap font-medium",
                                        "DEFAULT"
                                    }
                                }
                                match &reaction {
                ...
                if let Some(err) = error_msg.read().as_ref() {
                    div { class: "bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded p-3 mb-4",
