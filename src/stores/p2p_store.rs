...
impl Default for P2PFilterState {
    fn default() -> Self {
        Self {
            order_type: None,
            status: Some(OrderStatus::Pending), // Show active orders by default
            currency: None,
            payment_method: None,
            layer: None,
            network: None,
...
pub fn filter_orders(orders: &[P2POrder], filters: &P2PFilterState) -> Vec<P2POrder> {
    orders
        ...
        .filter(|order| {
            ...
            // Amount range filter (uses fiat amount overlap)
...
pub fn orders_filter_all() -> Filter {
    // 30 days ago in seconds (WASM-compatible using js_sys)
...
pub fn derive_market_price(orders: &[P2POrder]) -> Option<f64> {
    ...
    // Collect implied market prices from USD orders with explicit sats
    let mut implied_prices: Vec<f64> = orders
        .iter()
        // Filter: USD orders only, with explicit sats amount, active/pending
        .filter(|o| o.currency.eq_ignore_ascii_case("USD"))
        .filter(|o| o.amount_sats > 0)
        .filter(|o| o.is_active())
        .filter_map(|o| {
            // Get fiat amount (use midpoint for ranges)
            ...
            // Calculate raw price (fiat per BTC)
            let raw_price = fiat / btc;

            // Adjust for premium to get implied "market" price
            // If order has +5% premium, order_price = market * 1.05
            // So market = order_price / 1.05
    ...
    // Need at least 1 valid order to derive price
    if implied_prices.is_empty() {
        return None;
    }

    // Sort and return median (more robust than mean)
