...
fn extract_root_from_event(
    ...
    if let Some(result) = event
        ...
        .find_map(|tag| {
            if let Some(
                nostr::TagStandard::Event { event_id, relay_url, marker, public_key, .. },
            ) = tag.as_standardized()
            {
...
fn validate_voice_params(
    ...
    let capped_waveform = if waveform.len() > MAX_WAVEFORM_LEN {
        log::warn!(
            "Waveform truncated from {} to {} samples", waveform.len(), MAX_WAVEFORM_LEN
        );
...
pub async fn publish_picture_tracked(
    ...
    for url in &image_urls {
        ...
        tags.push(Tag::custom(nostr::TagKind::Custom("imeta".into()), imeta_fields));
    }
    if !location.is_empty() {
        tags.push(
            Tag::custom(nostr::TagKind::Custom("location".into()), vec![location]),
        );
    }
    ...
    log::info!(
        "Picture '{}' published: {} ({}/{} relays succeeded)", title, result.event_id,
...
pub async fn publish_voice_message_tracked(
    ...
    let result = PublishResult::from_output(output);
    log::info!(
        "Voice message published: {} ({}/{} relays succeeded)", result.event_id, result
...
pub async fn publish_voice_message_reply_tracked(
    ...
    let url = nostr::Url::parse(&audio_url)
        .map_err(|e| format!("Invalid audio URL: {}", e))?;
    let (root_event_id, root_pubkey, root_relay_url) = extract_root_from_event(
        &reply_to,
    );
    ...
    let mut builder = nostr::EventBuilder::voice_message_reply(
        url,
        root_target,
        parent_target,
    );
    ...
    let result = PublishResult::from_output(output);
    log::info!(
        "Voice message reply published: {} ({}/{} relays succeeded)", result.event_id,
        result.success_count(), result.total_attempted()
    );
    if result.has_failures() {
        for (relay, error) in &result.failed_relays {
...
pub async fn publish_voice_message_reply(
    ...
    publish_voice_message_reply_tracked(
            audio_url,
            duration,
            waveform,
            reply_to,
