...
pub fn get_available_items(active_items: &[SidebarItem]) -> Vec<SidebarItem> {
    let active_set: std::collections::HashSet<_> = active_items
        .iter()
        .cloned()
        .collect();
    SidebarItem::iter().filter(|item| !active_set.contains(item)).collect()
}
...
pub fn init_sidebar_from_cache() {
    if let Some(cached) = load_cached_sidebar() {
        if !cached.active_items.is_empty() {
            log::info!(
                "Initialized {} sidebar items from localStorage", cached.active_items
                .len()
            );
...
pub async fn load_sidebar_preferences() {
    ...
    if let Some(cached) = load_cached_sidebar() {
        log::info!(
            "Loaded {} sidebar items from localStorage", cached.active_items.len()
        );
        *SIDEBAR_ITEMS.write() = cached.active_items;
        *SIDEBAR_SLOT_COUNT.write() = cached.main_sidebar_count;
...
pub async fn save_sidebar_preferences(
    ...
    log::info!(
        "Saving {} sidebar items with {} main slots to Nostr (NIP-78)...", items.len(),
        slot_count
    );
    if !auth_store::is_authenticated() {
