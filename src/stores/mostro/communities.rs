...
//!
//! Phase 6.5: the relay lists in `COMMUNITIES` are **hint-only**. The
//! authoritative relay list for the selected daemon comes from its live
//! NIP-65 (kind 10002) relay list, fetched by
//! `node_config::sync_relays_from_nip65`. The static relays here are only
//! used as a fallback during initial discovery (before the NIP-65 fetch
//! completes). Real daemons update their relay lists over time; the static
//! entries may drift from reality and should never override live data.
