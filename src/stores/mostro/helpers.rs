...
/// Extract a peer's trade pubkey from a `Payload::Peer`.
///
/// Used for `Action::AdminTakeDispute` / `AdminTookDispute` (the daemon sends
/// the solver's pubkey inside `Payload::Peer.pubkey`, NOT as the gift-wrap
/// rumor sender) and for `Action::FiatSentOk` (counterparty reputation
/// disclosure). Returns `None` for non-`Peer` payloads or unparseable pubkeys.
pub fn extract_peer_pubkey(payload: &Option<mostro_core::prelude::Payload>) -> Option<PublicKey> {
    use mostro_core::prelude::Payload;
    match payload {
        Some(Payload::Peer(peer)) => parse_node_pubkey(&peer.pubkey).ok(),
        _ => None,
    }
}
