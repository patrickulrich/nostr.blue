...
/// decrypts/deserializes to a valid `P2PTermsAcceptance` with version >= current.
///
/// Tries NIP-44 decryption first (new encrypted format). If that fails,
/// falls back to plaintext JSON (legacy events published before the
/// encryption upgrade). This dual-path is provided by
/// `private_app_data::decrypt_from_self_or_legacy`.
///
...
fn evaluate_event(event: &NostrEvent, user_pubkey: &PublicKey) -> Option<u32> {
    ...
    // Try encrypted (new format) first; falls back to plaintext (legacy)
    // automatically via decrypt_from_self_or_legacy.
    let parsed: P2PTermsAcceptance =
...
///
/// The event content is NIP-44 encrypted to self (consistent with
/// `trade_store`, `node_config`, and `p2p_settings`), so that the
/// acceptance timestamp and version are not leaked publicly.
#[allow(dead_code)]
pub async fn accept_p2p_terms() -> Result<(), String> {
    ...
    // Use the encrypted event builder (same pattern as trade_store and
    // node_config). Falls back to plaintext if Mostro keys aren't
    // initialized — the read path (`evaluate_event` via
    // `decrypt_from_self_or_legacy`) handles both formats.
    let builder = match crate::stores::private_app_data::build_encrypted_event_builder(
        ...
        Ok(b) => b,
        Err(e) => {
            log::warn!("Falling back to plaintext terms event: {e}");
            let content = serde_json::to_string(&payload)
                .map_err(|e| format!("Failed to serialize terms: {e}"))?;
            EventBuilder::new(Kind::from(30078), content).tag(Tag::identifier(P2P_TERMS_D_TAG))
        }
    };
