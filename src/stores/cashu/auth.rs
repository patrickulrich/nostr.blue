    ...
    if let Some(auth_required) = auth_state.endpoint_requires_auth(endpoint) {
        match auth_required {
            ...
            AuthRequired::Blind => {
                ...
                Err(
                    "Blind auth required but no tokens available. Please request blind auth tokens."
                        .to_string(),
                )
            }
        }
    ...
    match auth_state.endpoint_requires_auth(endpoint) {
        Some(AuthRequired::Clear) => {
            if let Some(ref token) = auth_state.clear_token {
                ...
                Err(
                    "Clear auth required but no token available. Please authenticate with the mint."
                        .to_string(),
                )
            }
        }
    ...
    match auth_required {
        Some(AuthRequired::Clear) => {
            ...
            if state.clear_token.is_none() {
                return Err(
                    "This mint requires Clear Auth (NUT-21). Please authenticate with the mint's OAuth provider."
                        .to_string(),
                );
            }
        }
...
mod tests {
    ...
    fn test_is_protected_mint() {
        let protected = serde_json::json!({ "name" : "Test", "nuts" : { "21" : {} } });
        assert!(is_protected_mint(&protected));
        let protected22 = serde_json::json!({ "name" : "Test", "nuts" : { "22" : {} } });
        assert!(is_protected_mint(&protected22));
        let unprotected = serde_json::json!({ "name" : "Test", "nuts" : {} });
