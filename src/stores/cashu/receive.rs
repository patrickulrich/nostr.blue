...
pub async fn receive_tokens_with_options(
    ...
    let token_string = sanitize_and_validate_token(&token_string)?;
    log::info!(
        "Token string length: {}, starts with: {}", token_string.len(), token_string
        .chars().take(10).collect::< String > ()
    );
    if token_string.chars().any(|c| c.is_control()) {
        log::warn!("Token contains control characters");
        return Err(
            "Token contains invalid control characters. Please copy the token again."
                .to_string(),
        );
    }
    let base64_part = if token_string.starts_with("cashuA")
        || token_string.starts_with("cashuB")
    {
        &token_string[6..]
    } else {
        ""
    ...
    log::info!(
        "Base64 portion length: {}, last 20 chars: {}", base64_part.len(), base64_part
        .chars().rev().take(20).collect::< String > ().chars().rev().collect::< String >
        ()
    );
    let remainder = base64_part.len() % 4;
    let token_to_parse = if remainder != 0 {
        log::warn!(
            "Base64 portion length {} is not a multiple of 4. Remainder: {}", base64_part
            .len(), remainder
        );
            ...
            if let Ok(proofs) = token.proofs(&keysets) {
                if let Some(first_proof) = proofs.first() {
                    ...
                    if let Some(keyset_info) = keysets.iter().find(|k| k.id == keyset_id)
                    {
                        ...
                        log::warn!(
                            "Keyset {} not found on mint {}", keyset_id, mint_url
                        );
                    }
                }
            }
            ...
            Err(e) => {
                ...
                match &e {
                    CdkError::DleqProofNotProvided => {
                        log::warn!(
                            "Token does not contain DLEQ proofs - cannot verify offline"
                        );
                        return Err(
                    ...
                    CdkError::CouldNotVerifyDleq => {
                        ...
                        return Err(
                            "Token verification failed: Invalid DLEQ proof signature."
                                .to_string(),
                        );
                    }
                    _ => {
        ...
        if let Ok(keysets) = wallet.get_mint_keysets().await {
            if let Ok(proofs) = token.proofs(&keysets) {
                for proof in &proofs {
                    ...
                    let spending_conditions: Option<SpendingConditions> = (&proof.secret)
                        .try_into()
                        .ok();
                    if let Some(conditions) = spending_conditions {
                        if let Some(pubkeys) = conditions.pubkeys() {
                            for pubkey in pubkeys {
                                log::debug!(
                                    "Token P2PK locked to pubkey: {}", hex::encode(pubkey
    ...
    if !options.preimages.is_empty() {
        log::info!(
            "Using {} HTLC preimages for receive (NUT-14)", options.preimages.len()
        );
    }
    ...
    let amount_received = match wallet.receive(&token_to_parse, receive_opts).await {
        ...
        Err(e) => {
            if is_token_already_spent_error(&e) {
                log::warn!(
                    "Token already spent or redeemed, checking for spent proofs in wallet"
                );
                match cleanup_spent_proofs_internal(&mint_url).await {
                    Ok((cleaned_count, cleaned_amount)) if cleaned_count > 0 => {
                        log::info!(
                            "Cleaned up {} spent proofs worth {} sats", cleaned_count,
                            cleaned_amount
                        );
                        return Err(
                            format!(
                    ...
                    Ok(_) => {
                        return Err(
                            "This token has already been spent and cannot be redeemed."
                                .to_string(),
                        );
                    }
                    Err(cleanup_err) => {
        ...
        match client.send_event(&signed_event).await {
            Ok(output) => {
                if !output.success.is_empty() {
                    ...
                    log::info!(
                        "Published token event to {}/{} relays", output.success.len(),
    ...
    let event_id = match event_id {
        ...
        None => {
            if retryable {
                log::error!(
                    "All publish attempts failed, queueing for retry: {}", last_error
                );
                let pending_id = format!("pending_{}", uuid::Uuid::new_v4());
                let retry_builder = nostr_sdk::EventBuilder::new(
                    Kind::CashuWalletUnspentProof,
                    encrypted.clone(),
                );
