                    ...
                    match entry {
                        ...
                        DirectoryEntry::File { event_id, .. } => {
                            Some(DirectoryNode {
                                ...
                                name: event_id[..8].to_string(), // Shortened ID as name
                                is_directory: false,
                                children: vec![],
                                depth: depth + 1,
                                expanded: false,
        ...
        match slice.first().map(|s| s.as_str()) {
            ...
            Some("a") => {
                if let Some(addr) = slice.get(1) {
                    // Only add 30045 directory references
...
pub fn parse_symlink_event(event: &NostrEvent) -> Option<SymlinkEvent> {
    ...
    for tag in event.tags.iter() {
        ...
        match slice.first().map(|s| s.as_str()) {
            ...
            Some("A") => {
                // First A is context, second is drive
        ...
        if let DirectoryEntry::Subdirectory { address, .. } = entry {
            if !tree.directories.contains_key(address) {
                if let Ok(Some(subdir)) = fetch_directory_by_address(address).await {
                    ...
                    // Recursively fetch deeper subdirectories
