...
use crate::utils::nip99::{
    ...
    OrderMessageType, ProductImage, KIND_PRODUCT, KIND_COLLECTION, KIND_SHIPPING, KIND_REVIEW,
...
const COLLECTION_CACHE_SIZE: usize = 50;
...
// ============================================================================
// Global Signals - Collections
// ============================================================================

/// Collection cache (keyed by naddr string)
pub static COLLECTIONS_CACHE: GlobalSignal<LruCache<String, ProductCollection>> =
    GlobalSignal::new(|| LruCache::new(NonZeroUsize::new(COLLECTION_CACHE_SIZE).unwrap()));
...
/// Get a product from cache by coordinate string
pub fn get_cached_product_by_coordinate(coordinate: &str) -> Option<Product> {
    let cache = PRODUCTS_CACHE.read();
    cache
        .iter()
        .find(|(_, product)| product.coordinate == coordinate)
...
pub fn get_all_cached_products() -> Vec<Product> {
    let cache = PRODUCTS_CACHE.read();
    cache.iter().map(|(_, product)| product.clone()).collect()
}

/// Get visible products only (on-sale or pre-order)
pub fn get_visible_products() -> Vec<Product> {
    let cache = PRODUCTS_CACHE.read();
    cache
        .iter()
...
/// Get a collection from cache by naddr
pub fn get_cached_collection(naddr: &str) -> Option<ProductCollection> {
    COLLECTIONS_CACHE.read().peek(naddr).cloned()
}

/// Cache a collection
pub fn cache_collection(collection: ProductCollection) {
    COLLECTIONS_CACHE.write().put(collection.naddr.clone(), collection);
}

/// Parse and cache collections from events
pub fn cache_collection_events(events: &[NostrEvent]) {
    let mut cache = COLLECTIONS_CACHE.write();
    for event in events {
        if let Ok(collection) = parse_collection(event) {
            cache.put(collection.naddr.clone(), collection);
        }
    }
}
...
/// Parse and cache shipping options from events
pub fn cache_shipping_events(events: &[NostrEvent]) {
    let mut cache = SHIPPING_CACHE.write();
    for event in events {
        if let Ok(shipping) = parse_shipping(event) {
            cache.put(shipping.naddr.clone(), shipping);
        }
    }
}
...
fn recalculate_cart_total() {
    ...
    for item in items.iter() {
        ...
        // In production, would need to convert fiat prices using exchange rate
        let price = item.product.price.amount;
        if item.product.price.currency.eq_ignore_ascii_case("sats")
            || item.product.price.currency.eq_ignore_ascii_case("sat") {
            total += (price as u64) * item.quantity as u64;
        } else {
            // For fiat prices, we'd need to convert - for now use as-is
            // This should be improved with proper exchange rate conversion
            total += (price * 100.0) as u64 * item.quantity as u64; // Rough approximation
        }
...
/// Build filter for fetching ALL products
/// Uses a 30-day lookback for performance
pub fn products_filter_all() -> Filter {
...
/// Build filter for fetching products by category
pub fn products_filter_by_category(category: &str, limit: usize) -> Filter {
    Filter::new()
        .kind(Kind::Custom(KIND_PRODUCT))
        .hashtag(category)
        .limit(limit)
...
/// Build filter for fetching collections
pub fn collections_filter(limit: usize) -> Filter {
    Filter::new()
        .kind(Kind::Custom(KIND_COLLECTION))
        .limit(limit)
}

/// Build filter for fetching shipping options
pub fn shipping_filter(limit: usize) -> Filter {
    Filter::new()
        .kind(Kind::Custom(KIND_SHIPPING))
        .limit(limit)
}
...
/// Fetch ALL products (30-day lookback)
pub async fn fetch_all_products() -> Result<Vec<Product>> {
    *LOADING_PRODUCTS.write() = true;

    let filter = products_filter_all();
    let result =
        crate::stores::nostr_client::fetch_events_aggregated(filter, Duration::from_secs(30))
            .await;
    ...
    match result {
        Ok(events) => {
            ...
            let products: Vec<Product> = events
                ...
                .filter_map(|e| parse_product(e).ok())
                .collect();
            log::info!("Fetched {} products for marketplace", products.len());
            Ok(products)
        }
        Err(e) => Err(e),
    }
...
impl OrderMessageContent {
    ...
    /// Get the numeric type value for market-spec compatibility
    pub fn type_as_u8(&self) -> Option<u8> {
        self.get_type().map(|t| t.as_u8())
    }

    /// Check if this is an order creation message
    pub fn is_order(&self) -> bool {
        self.get_type() == Some(OrderMessageType::OrderCreation)
    }

    /// Check if this is a payment message
    pub fn is_payment(&self) -> bool {
        self.get_type() == Some(OrderMessageType::PaymentRequest)
    }

    /// Check if this is a status update message
    pub fn is_status(&self) -> bool {
        self.get_type() == Some(OrderMessageType::StatusUpdate)
    }

    /// Check if this is a shipping update message
    pub fn is_shipping(&self) -> bool {
        self.get_type() == Some(OrderMessageType::ShippingUpdate)
    }
    ...
    pub fn new_message(order_id: &str, message: &str) -> Self {
        Self {
            message_type: OrderMessageType::Message.as_str().to_string(),
            order_id: order_id.to_string(),
            payload: serde_json::json!({
                "text": message,
            }),
...
pub async fn create_shop_order(
    ...
    // Generate order ID
    let order_id = format!("{:x}", std::time::SystemTime::now()
...
impl ProductFormData {
    ...
    /// Uses ProductImage::new() constructor for clean image creation
    pub fn get_product_images(&self) -> Vec<ProductImage> {
        self.images.iter()
            .filter(|url| !url.trim().is_empty())
            .map(|url| ProductImage::new(url))
            .collect()
    }
}
...
pub async fn publish_product(data: ProductFormData) -> Result<String> {
    ...
    // Generate d-tag (unique identifier)
    let d_tag = format!("{:x}", std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_millis());
...
pub async fn publish_review(
    ...
    if let Some(v) = value_rating {
        tags.push(Tag::custom(TagKind::custom("rating"), vec![
            "value".to_string(), format!("{:.1}", v), "5".to_string()
        ]));
    }
    if let Some(q) = quality_rating {
        tags.push(Tag::custom(TagKind::custom("rating"), vec![
            "quality".to_string(), format!("{:.1}", q), "5".to_string()
        ]));
    }
    if let Some(d) = delivery_rating {
        tags.push(Tag::custom(TagKind::custom("rating"), vec![
            "delivery".to_string(), format!("{:.1}", d), "5".to_string()
        ]));
    }
    if let Some(c) = communication_rating {
        tags.push(Tag::custom(TagKind::custom("rating"), vec![
            "communication".to_string(), format!("{:.1}", c), "5".to_string()
        ]));
    }
...
pub fn process_order_message(msg: &OrderMessageContent) -> Result<()> {
    ...
    match msg.get_type() {
        Some(OrderMessageType::OrderCreation) => {
            ...
            // The order would be parsed from the payload and added to SELLER_ORDERS
            // This is handled by the checkout flow sending to merchant
        }
        Some(OrderMessageType::PaymentRequest) => {
            ...
            if let Some(order) = BUYER_ORDERS.write().iter_mut().find(|o| o.order_id == *order_id) {
            ...
            Ok(unwrapped) => {
                ...
                if kind_num == KIND_ORDER_MESSAGE || kind_num == KIND_PAYMENT_RECEIPT {
                    ...
                    match serde_json::from_str::<OrderMessageContent>(&rumor.content) {
                        Ok(msg) => {
                            if let Err(e) = process_order_message(&msg) {
...
pub async fn update_product(d_tag: &str, data: ProductFormData) -> Result<String> {
    ...
    for cat in data.categories.iter() {
        if !cat.is_empty() {
            tags.push(Tag::custom(TagKind::custom("t"), vec![cat.clone()]));
        }
    }
    ...
    if !data.is_digital {
        for region in data.shipping_regions.iter() {
            if !region.is_empty() {
                tags.push(Tag::custom(TagKind::custom("ships_to"), vec![region.clone()]));
            }
        }
    }
