...
impl PendingComment {
    ...
    pub fn to_display_event(&self) -> NostrEvent {
        ...
        let dummy_sig = Signature::from_str(
            ...
            .unwrap_or_else(|e| {
                log::error!(
                    "Failed to create dummy signature (should never happen): {}", e
                );
                Signature::from_slice(&[0u8; 64])
                    .expect("64 zero bytes is valid signature format")
            });
        NostrEvent::new(
            id,
            self.author_pubkey,
...
pub static PENDING_COMMENTS: GlobalSignal<HashMap<String, Vec<PendingComment>>> = Signal::global(
    HashMap::new,
);
...
pub fn get_pending_comments(target_event_id: &EventId) -> Vec<PendingComment> {
    PENDING_COMMENTS.read().get(&target_event_id.to_hex()).cloned().unwrap_or_default()
}
...
pub fn retry_pending_comment(local_id: &str) {
    ...
    let Some((local_id, content, kind, target_event, parent_comment, target_event_id)) = comment_data
    else {
    ...
    spawn_forever(async move {
        if kind == Kind::Comment {
            ...
            let root_target = root
                .map(|r| CommentTarget::event(r.id, r.kind, None, None));
            let builder = EventBuilder::comment(&content, comment_target, root_target);
            match client.send_event_builder(builder).await {
                Ok(send_output) => {
                    log::info!(
                        "NIP-22 comment retry successful: {}", send_output.id().to_hex()
                    );
            ...
            if let Some(root_id) = parent_root {
                tags.push(
                    vec!["e".to_string(), root_id, "".to_string(), "root".to_string()],
                );
                tags.push(
                    vec![
                        "e".to_string(),
            ...
            for tag in target_event.tags.iter() {
                let tag_slice = tag.as_slice();
                if tag_slice.len() >= 2
                    && tag_slice.first().map(|s| s.as_str()) == Some("p")
                {
            ...
            match publish_note(content, tags).await {
                Ok(published_event_id) => {
                    ...
                    match EventId::from_hex(&published_event_id) {
                        ...
                        Err(e) => {
                            log::error!(
                                "Failed to parse event ID '{}': {}", published_event_id, e
                            );
                            update_pending_status(
                                &local_id,
