...
pub static CODE_ISSUES_CACHE: GlobalSignal<LruCache<String, Issue>> =
    GlobalSignal::new(|| LruCache::new(NonZeroUsize::new(ISSUE_CACHE_SIZE).unwrap()));
/// Pull Request cache (keyed by event ID hex)
pub static CODE_PRS_CACHE: GlobalSignal<LruCache<String, PullRequest>> =
    GlobalSignal::new(|| LruCache::new(NonZeroUsize::new(PR_CACHE_SIZE).unwrap()));
/// Code Snippet cache (keyed by event ID hex)
pub static CODE_SNIPPETS_CACHE: GlobalSignal<LruCache<String, DisplaySnippet>> =
    GlobalSignal::new(|| LruCache::new(NonZeroUsize::new(SNIPPET_CACHE_SIZE).unwrap()));
/// Discussion cache (keyed by event ID hex)
pub static CODE_DISCUSSIONS_CACHE: GlobalSignal<LruCache<String, Discussion>> =
    GlobalSignal::new(|| LruCache::new(NonZeroUsize::new(DISCUSSION_CACHE_SIZE).unwrap()));
/// Release cache (keyed by event ID hex)
pub static CODE_RELEASES_CACHE: GlobalSignal<LruCache<String, Release>> =
    GlobalSignal::new(|| LruCache::new(NonZeroUsize::new(RELEASE_CACHE_SIZE).unwrap()));
/// Bounty cache (keyed by issue_event_id, stores Vec<Bounty>)
pub static CODE_BOUNTIES_CACHE: GlobalSignal<LruCache<String, Vec<Bounty>>> =
    GlobalSignal::new(|| LruCache::new(NonZeroUsize::new(BOUNTY_CACHE_SIZE).unwrap()));
...
pub static STARRED_REPOS: GlobalSignal<HashSet<String>> = GlobalSignal::new(HashSet::new);
...
pub fn get_repo_issues(repo_naddr: &str) -> Vec<Issue> {
    ...
    cache
        .iter()
        .filter_map(|(_, issue)| {
            if issue.repository_naddr == repo_naddr {
                Some(issue.clone())
            } else {
                None
            }
...
pub fn get_repo_prs(repo_naddr: &str) -> Vec<PullRequest> {
    ...
    cache
        .iter()
        .filter_map(|(_, pr)| {
            if pr.repository_naddr == repo_naddr {
                Some(pr.clone())
            } else {
                None
            }
...
pub fn cache_snippet(snippet: DisplaySnippet) {
    CODE_SNIPPETS_CACHE
        .write()
        .put(snippet.event_id.clone(), snippet);
}
...
pub fn get_user_snippets() -> Vec<DisplaySnippet> {
    ...
    ids.iter()
        .filter_map(|id| cache.peek(id).cloned())
        .collect()
}
...
pub fn cache_bounty_events(events: &[NostrEvent]) {
    ...
    for event in events {
        if let Some(bounty) = Bounty::from_event(event) {
            grouped
                .entry(bounty.issue_event_id.clone())
                .or_default()
                .push(bounty);
        }
    }
...
pub fn cache_persisted_review_events(events: &[NostrEvent]) {
    ...
    for event in events {
        if let Some(review) = PersistedReview::from_event(event) {
            grouped
                .entry(review.pr_event_id.clone())
                .or_default()
                .push(review);
        }
    }
