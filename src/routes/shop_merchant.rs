        ...
        div { class: "min-h-screen",
            div { class: "sticky top-0 z-10 bg-background/80 backdrop-blur-sm border-b border-border",
                ...
                div { class: "flex border-b border-border",
                    button {
                        class: if *active_tab.read() == MerchantTab::Products { "flex-1 py-3 text-center font-medium border-b-2 border-blue-500 text-blue-500" } else { "flex-1 py-3 text-center text-muted-foreground hover:text-foreground transition" },
                        onclick: move |_| active_tab.set(MerchantTab::Products),
                        "Products ({product_count})"
                    }
                    button {
                        class: if *active_tab.read() == MerchantTab::Collections { "flex-1 py-3 text-center font-medium border-b-2 border-blue-500 text-blue-500" } else { "flex-1 py-3 text-center text-muted-foreground hover:text-foreground transition" },
                        onclick: move |_| active_tab.set(MerchantTab::Collections),
                        "Collections ({collection_count})"
                    }
                    button {
                        class: if *active_tab.read() == MerchantTab::Stats { "flex-1 py-3 text-center font-medium border-b-2 border-blue-500 text-blue-500" } else { "flex-1 py-3 text-center text-muted-foreground hover:text-foreground transition" },
                                ...
                                div {
                                    ...
                                    div { class: "flex items-start gap-4 p-4",
                                        ...
                                        div { class: "flex-1 min-w-0",
                                            ...
                                            {
                                                let price_sats = if product.price.currency.eq_ignore_ascii_case("sats")
                                                    || product.price.currency.eq_ignore_ascii_case("sat")
                                                {
                                                ...
                                                if price_sats > 0 {
                                                    rsx! {
                                                        p { class: "text-amber-500 font-medium", "⚡{price_sats} sats" }
                                                    }
                                                } else {
                                                    rsx! {
                                                        p { class: "font-medium", "{product.price.display()}" }
                                                    }
                                                }
                                            }
                                            div { class: "flex items-center gap-2 mt-1",
                                                span { class: "text-xs px-2 py-0.5 bg-muted rounded",
                                                    if product.format.is_digital() {
                                                        "Digital"
                                                    } else {
                                                        "Physical"
                                                    }
                                                ...
                                                if let Some(stock) = product.stock {
                                                    span { class: if stock == 0 { "text-xs px-2 py-0.5 bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400 rounded" } else if stock < 5 { "text-xs px-2 py-0.5 bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 rounded" } else { "text-xs px-2 py-0.5 bg-muted rounded" },
                                                        "Stock: {stock}"
                                                    }
                                                }
                                            }
