    ...
    rsx! {
        div { class: "min-h-screen",
            ...
            if !*nostr_client::CLIENT_INITIALIZED.read()
                || (*loading.read() && recipes.read().is_empty())
            {
                div { class: "p-4",
                    div { class: "flex items-start gap-4 mb-8 p-4 bg-card border border-border rounded-xl animate-pulse",
                ...
                div { class: "p-4",
                    div { class: "flex flex-col items-center justify-center py-12 text-center",
                        ...
                        h2 { class: "text-xl font-semibold mb-2", "Chef Not Found" }
                        p { class: "text-muted-foreground mb-4", "Invalid public key format" }
                        Link {
                            to: Route::RecipesHome {},
                ...
                div { class: "p-4",
                    div { class: "flex items-start gap-4 mb-8 p-4 bg-card border border-border rounded-xl",
                        div { class: "w-20 h-20 rounded-full overflow-hidden bg-muted flex items-center justify-center shrink-0",
                    ...
                    h2 { class: "text-lg font-semibold mb-4", "Recipes by {display_name}" }
                    if let Some(err) = error.read().as_ref() {
                        ...
                        div { class: "flex flex-col items-center justify-center py-12 text-center",
                            ...
                            h3 { class: "text-lg font-medium mb-1", "No recipes yet" }
                            ...
                            div { class: "flex justify-center pt-6",
                                button {
                                    ...
                                    if *pagination_loading.read() {
                                        span { class: "inline-block w-4 h-4 border-2 border-primary border-t-transparent rounded-full animate-spin mr-2" }
                                        "Loading..."
                                    } else {
                                        "Load More"
                                    }
