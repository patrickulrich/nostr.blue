        ...
        div { class: "max-w-6xl mx-auto p-4 space-y-6",
                ...
                {
                    {
                        ...
                        rsx! {
                            button {
                                key: "{tab:?}",
                                class: if is_active { "px-4 py-3 font-medium border-b-2 border-blue-500 text-blue-500" } else { "px-4 py-3 font-medium text-muted-foreground hover:text-foreground transition" },
                                onclick: move |_| {
                                    active_tab.set(tab);
                                    selected_items.write().clear();
                                },
                                "{tab.label()}"
                                if count > 0 {
                                    span { class: "ml-2 text-xs bg-muted px-2 py-0.5 rounded-full", "{count}" }
                                }
                            }
                        }
                    }
            ...
            if active_tab() == BlossomTab::Servers {
                ...
                div { class: "flex flex-col items-center justify-center py-16",
                    ...
                    p { class: "text-muted-foreground font-medium",
                        "No {active_tab().label().to_lowercase()} found"
                    }
            ...
            if selected_count > 0 {
                div { class: "fixed bottom-4 left-1/2 -translate-x-1/2 bg-card border border-border rounded-lg shadow-lg p-4 flex items-center gap-4 z-50",
                    span { class: "text-sm font-medium", "{selected_count} selected" }
        ...
        if let Some(file) = selected_file() {
            FileDetailModal {
                ...
                on_mirror: move |item: MediaItem| {
                    spawn(async move {
                        if let Err(e) = blossom_store::mirror_file(&item.sha256, &item.url, None)
                            .await
                        {
...
fn MediaCard(
    ...
    rsx! {
        div {
            ...
            div { class: if is_selected { "aspect-square rounded-lg overflow-hidden border-2 border-blue-500 bg-muted" } else { "aspect-square rounded-lg overflow-hidden border border-border bg-muted hover:border-blue-500/50 transition" },
                if is_image {
                    img {
            ...
            div {
                ...
                div { class: if is_selected { "w-6 h-6 rounded-md bg-blue-500 flex items-center justify-center" } else { "w-6 h-6 rounded-md border-2 border-white/80 bg-black/30 group-hover:bg-black/50 transition" },
                    if is_selected {
                        svg {
            ...
            if mirror_count > 0 {
                div { class: if is_fully_mirrored { "absolute top-2 right-2 px-2 py-0.5 text-xs font-medium rounded-full bg-green-500 text-white" } else { "absolute top-2 right-2 px-2 py-0.5 text-xs font-medium rounded-full bg-yellow-500 text-white" },
                    "{mirror_count + 1}"
                }
            }
                            ...
                            div { class: "flex items-center gap-3",
                                ...
                                div { class: "flex-1 min-w-0",
                                    ...
                                    p { class: "text-sm text-muted-foreground",
                                        "{blossom_store::format_bytes(data.len() as u64)}"
                                    }
                                }
                                button {
                                    class: "p-2 hover:bg-accent rounded-lg transition",
            ...
            div { class: "pt-4 border-t border-border",
                div { class: "flex items-center justify-between",
                    div {
                        ...
                        p { class: "text-sm text-muted-foreground",
                            "Save your server preferences to Nostr (kind 10063)"
                        }
                    }
                    button {
                        class: "px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition disabled:opacity-50",
                    ...
                    {
                        {
                            ...
                            rsx! {
                                button {
                                    key: "{url}",
                                    class: if is_added { "px-3 py-1.5 text-sm bg-muted text-muted-foreground rounded-lg cursor-not-allowed" } else { "px-3 py-1.5 text-sm bg-muted hover:bg-accent rounded-lg transition" },
                                    disabled: is_added,
                                    onclick: {
