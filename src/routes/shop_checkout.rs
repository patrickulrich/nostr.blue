                    ...
                    match *step.read() {
                                            ...
                                            div { class: "text-right",
                                                {
                                                    ...
                                                    rsx! {
                                                        p { class: "font-medium text-amber-500", "⚡{format_sats_with_separator(item_price)}" }
                                                    }
                                                }
                                            }
                                        }
                                ...
                                div { class: "space-y-3",
                                    {
                                        ...
                                        rsx! {
                                            button {
                                                class: if is_selected { "w-full p-4 bg-card border-2 border-blue-500 rounded-lg text-left" } else if lightning_available { "w-full p-4 bg-card border border-border rounded-lg text-left hover:border-ring transition" } else { "w-full p-4 bg-card border border-border rounded-lg text-left opacity-60 cursor-not-allowed" },
                                                    ...
                                                    div { class: "flex-1",
                                                        p { class: "font-medium",
                                                            ...
                                                            if !lightning_available && !merchants_missing.is_empty() {
                                                                span { class: "text-xs text-destructive ml-2", "(Not available)" }
                                                            }
                                                        }
                                                        if !lightning_available && !merchants_missing.is_empty() {
                                                ...
                                                MultiMerchantPaymentDisplay {
                                                    ...
                                                    on_payment_confirmed: move |preimage: String| {
                                                        ...
                                                        if all_paid {
                                                            ...
                                                            let all_preimages: Vec<String> = updated
                                                                .iter()
                                                            ...
                                                            if next_idx < updated.len() {
                                                                ...
                                                                spawn(async move {
                                                                    match lnurl::get_invoice_from_lud16(
                                                                            &next.lud16,
                                                                            next.amount_sats,
                                                                            Some(&format!("Order: {}", next.name)),
                                                                        )
                                                                        .await
                                                                    {
                                                                        Ok(invoice) => {
                                                                        ...
                                                                        Err(e) => {
                                                                            payment_error
                                                                                .set(
                                                                                    Some(
                                                                                        format!("Failed to get invoice from {}: {}", next.name, e),
                                                                                    ),
                                                                                );
                                                                            show_invoice_qr.set(false);
                                                                            payment_processing.set(false);
                                                        ...
                                                        match method {
                                                            ...
                                                            PaymentMethod::Lightning => {
                                                                ...
                                                                if payments.is_empty() {
                                                                    payment_error
                                                                        .set(
                                                                            Some("No merchants with Lightning addresses".to_string()),
                                                                        );
                                                                    payment_processing.set(false);
                                                                    return;
                                                                }
                                                                if is_multi {
                                                                    log::info!(
                                                                        "Multi-merchant mode: {} merchants to pay", payments.len()
                                                                    );
                                                                    current_merchant_idx.set(0);
                                                                    let first = &payments[0];
                                                                    log::info!(
                                                                        "Getting invoice from {} for {} sats", first.name, first
                                                                        .amount_sats
                                                                    );
                                                                    match lnurl::get_invoice_from_lud16(
                                                                            &first.lud16,
                                                                            first.amount_sats,
                                                                            Some(&format!("Order: {}", first.name)),
                                                                        )
                                                                        .await
                                                                    {
                                                                        Ok(invoice) => {
                                                                                ...
                                                                                match nwc_store::pay_invoice(invoice.clone()).await {
                                                                                    Ok(response) => {
                                                                                        ...
                                                                                        if payments.len() > 1 {
                                                                                                            ...
                                                                                                            match nwc_store::pay_invoice(invoice).await {
                                                                                                                Ok(resp) => {
                                                                                                                    ...
                                                                                                                    if all_paid {
                                                                                                                        ...
                                                                                                                        let all_preimages: Vec<String> = upd
                                                                                                                            .iter()
                                                                                                                            .filter_map(|p| p.preimage.clone())
                                                                                                                            .collect();
                                                                                                                        let proof = all_preimages.join(",");
                                                                                                                ...
                                                                                                                Err(e) => {
                                                                                                                    log::warn!(
                                                                                                                        "NWC auto-pay failed for merchant 2: {}, showing QR", e
                                                                                                                    );
                                                                                                                    show_invoice_qr.set(true);
                                                                                                                }
                                                                                                            }
                                                                                            ...
                                                                                            let all_preimages: Vec<String> = merchant_payments
                                                                                                .read()
                                                                                                .iter()
                                                                                                .filter_map(|p| p.preimage.clone())
                                                                                                .collect();
                                                                        ...
                                                                        Err(e) => {
                                                                            payment_error
                                                                                .set(
                                                                                    Some(
                                                                                        format!("Failed to get invoice from {}: {}", first.name, e),
                                                                                    ),
                                                                                );
                                                                            payment_processing.set(false);
                                                                        }
                                                                    ...
                                                                    let Some(lud16_addr) = lud16 else {
                                                                        payment_error
                                                                            .set(Some("Merchant has no Lightning address".to_string()));
                                                                        payment_processing.set(false);
                                                                        return;
                                                                    };
                                                                        ...
                                                                        .await
                                                                    {
                                                                        ...
                                                                        Err(e) => {
                                                                            payment_error
                                                                                .set(Some(format!("Failed to get invoice: {}", e)));
                                                                            payment_processing.set(false);
                                                                        }
                                                                    }
                                                                }
                        ...
                        CheckoutStep::Complete => rsx! {
                            div { class: "text-center py-12",
                                ...
                                p { class: "text-muted-foreground mb-6", "Your payment has been sent to the merchant." }
                                ...
                                if let Some(id) = order_id.read().as_ref() {
                                    p { class: "text-sm text-muted-foreground mb-6", "Order ID: {id}" }
                                }
                                div { class: "space-y-3",
                                    Link {
                                        to: Route::ShopOrders {},
...
fn MultiMerchantPaymentDisplay(
    ...
    rsx! {
        div { class: "bg-card border border-border rounded-lg p-6 space-y-4",
            div { class: "text-center",
                h3 { class: "text-lg font-semibold mb-2", "Pay Merchants ({paid_count}/{total_count})" }
                p { class: "text-muted-foreground text-sm",
            ...
            div { class: "flex justify-center gap-2 py-2",
                for (idx , payment) in payments.iter().enumerate() {
                    div {
                        key: "{payment.pubkey}",
                        class: if payment.paid { "w-8 h-8 rounded-full bg-green-500 text-white flex items-center justify-center text-sm" } else if idx == current_idx { "w-8 h-8 rounded-full bg-blue-500 text-white flex items-center justify-center text-sm animate-pulse" } else { "w-8 h-8 rounded-full bg-muted text-muted-foreground flex items-center justify-center text-sm" },
                        if payment.paid {
                            "✓"
                        } else {
                            "{idx + 1}"
            ...
            div { class: "space-y-2 py-2",
                for (idx , payment) in payments.iter().enumerate() {
                    div {
                        ...
                        class: if idx == current_idx && !payment.paid { "flex items-center justify-between p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg" } else if payment.paid { "flex items-center justify-between p-3 bg-green-500/10 border border-green-500/30 rounded-lg" } else { "flex items-center justify-between p-3 bg-muted/50 rounded-lg opacity-60" },
                    ...
                    if let Some(ref invoice) = payment.invoice {
                        ...
                        div { class: "text-center py-8",
                            ...
                            p { class: "text-muted-foreground",
                                "Getting invoice for {payment.name}..."
                            }
                        }
                    }
                }
                                                ...
                                                for opt in opts.iter() {
                                                    {
                                                        ...
                                                        rsx! {
                                                            button {
                                                                key: "{opt.naddr}",
                                                                class: if is_selected { "w-full p-3 bg-blue-500/10 border-2 border-blue-500 rounded-lg text-left" } else { "w-full p-3 bg-muted/50 border border-border rounded-lg text-left hover:border-ring transition" },
                                                                onclick: move |_| {
                                                                ...
                                                                div { class: "flex items-center justify-between",
                                                                    div {
                                                                        ...
                                                                        if !opt.countries.is_empty() {
                                                                            p { class: "text-xs text-muted-foreground", "Ships to: {opt.countries.join(\", \")}" }
                                                                        }
                                                                        if let Some(duration) = opt.display_duration() {
                                                                            p { class: "text-xs text-muted-foreground", "Est. delivery: {duration}" }
                                                                        }
                                                                    }
                                                                    div { class: "text-right",
                                                                        span { class: "text-amber-500 font-medium", "{opt.display_price()}" }
                                                                        if is_selected {
            ...
            if shipping_total > 0 {
                div { class: "bg-muted/50 rounded-lg p-4",
                    div { class: "flex justify-between",
                        ...
                        span { class: "text-amber-500 font-medium",
                            "⚡{format_sats_with_separator(shipping_total)} sats"
                        }
                    }
                }
            }
