...
use crate::stores::social::channel_store::{
    cache_channel, cache_channel_metadata, channel_creation_by_id_filter, channel_metadata_filter,
    decode_channel_id, get_cached_channel, get_channel_display_info, parse_channel_creation,
    parse_channel_metadata, Channel,
};
...
pub fn ChatDetail(channel_id: String) -> Element {
    ...
    use_effect(use_reactive(
        (&channel_id_for_effect, &*client_ready.read()),
        move |(cid, ready)| {
            if !ready {
                return;
            }
    ...
    let creator_pubkey = use_memo(move || channel_info.read().as_ref().map(|ch| ch.pubkey.clone()));
