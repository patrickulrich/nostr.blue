    ...
    rsx! {
        div { class: "max-w-3xl mx-auto p-4 space-y-6",
            ...
            div { class: "text-sm text-muted-foreground",
                if cached_count == 0 {
                    p { "No chapters cached yet. Browse some chapters first to enable search." }
                } else {
                    p {
                        "Searching {cached_count} cached chapters. "
                        span { class: "text-xs", "Browse more chapters to expand searchable content." }
                    }
                }
            }
            if *has_searched.read() {
                if results.read().is_empty() {
                    div { class: "text-center py-12",
                        ...
                        p { class: "text-muted-foreground font-medium", "No results found" }
                    ...
                    div { class: "space-y-3",
                        p { class: "text-sm text-muted-foreground", "{results.read().len()} results" }
...
fn find_match_byte_range(haystack: &str, needle: &str) -> Option<(usize, usize)> {
    ...
    let mut match_start_idx: Option<usize> = None;
...
fn SearchResultCard(result: BibleSearchResult, query: String) -> Element {
    ...
    rsx! {
        Link {
            ...
            p { class: "font-medium text-sm text-primary mb-1", "{reference}" }
            p { class: "text-sm text-muted-foreground line-clamp-3",
                {
                    if let Some((match_start, match_end)) = find_match_byte_range(
                        &result.text,
                        &query,
                    ) {
                        ...
                        if let Some((slice_match_start, slice_match_end)) = find_match_byte_range(
                            text_slice,
                            &query,
                        ) {
                            rsx! {
