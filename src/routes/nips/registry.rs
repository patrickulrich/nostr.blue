...
pub const SUPPORTED_SPECS: &[SupportedSpec] = &[
    ...
    // NIP-101e Fitness Workouts (draft): kind-1301 workout records in the
    // two observed wire forms (verb-tag activity form, template-coordinate
    // strength form), plus kind-33401 exercise templates (rendered via
    // coordinate references). The embedded notes are the spec reference.
    SupportedSpec {
        spec_type: SpecType::Nip,
        number: "101e",
    ...
    SupportedSpec {
        spec_type: SpecType::Nip,
        number: "A3",
        title: "Payment Targets (payto)",
        kinds: Some("10133"),
        upstream_url: NIP_URL_BASE,
        notes: Some(include_str!("content/nip_A3.md")),
        naddr: None,
    },
