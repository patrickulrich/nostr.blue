...
use crate::components::{TopicPostCard, TopicPostComposer, TopicSidebar, ThreadView};
use crate::stores::auth_store;
use crate::stores::nostr_client::HAS_SIGNER;
use crate::stores::profiles::prefetch_profiles;
use crate::stores::topic_store::{
    build_topic_thread_tree, fetch_post_by_id, fetch_post_replies,
    fetch_votes_batch, TopicPost, TopicThread, VoteCounts,
};
use dioxus::prelude::*;
    ...
    let _resource = use_resource(move || {
        ...
        async move {
            ...
            if let Ok(Some(fetched_post)) = fetch_post_by_id(&post_id).await {
                ...
                let mut pubkeys: Vec<String> = reply_posts.iter().map(|p| p.pubkey.clone()).collect();
