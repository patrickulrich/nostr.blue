...
pub fn CitationsHome() -> Element {
    ...
    // Load citations when component mounts or auth changes
            ...
            if let Some(pk) = pubkey {
                ...
                spawn(async move {
                    match fetch_citations_by_author(&pk_clone, 200).await {
                        Ok(_) => {
                            // Citations are stored in USER_CITATIONS global signal
                            loading.set(false);
                        }
                        Err(e) => {
    ...
    // Get citations to display (search results or tab filtered)
    ...
    rsx! {
        div {
            ...
            // Header
            header {
                class: "sticky top-0 z-10 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b border-border",
                ...
                div {
                    ...
                    div {
                        ...
                        // New Citation button (when authenticated)
                        if is_authenticated {
                            button {
                    ...
                    div {
                        ...
                        {[CitationsTab::All, CitationsTab::Internal, CitationsTab::External, CitationsTab::Hardcopy, CitationsTab::Prompt]
                            .iter()
                            .map(|tab| {
                                ...
                                rsx! {
                                    button {
                                        key: "{tab.label()}",
                                        class: if is_active {
                                            "flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-lg bg-primary text-primary-foreground"
                                        } else {
                                            "flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-lg text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
                                        },
                                        onclick: move |_| {
            ...
            main {
                ...
                // Empty state
                    ...
                    div {
                        ...
                        p {
                            class: "text-muted-foreground max-w-md mb-4",
                            if !search_query.read().is_empty() {
