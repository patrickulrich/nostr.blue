                    ...
                    BadgesTab::All => {
                        match nip58::fetch_recent_badges(50).await {
                            Ok(badges) => {
                                ...
                                has_more.set(true); // Assume more available
                            }
                            Err(e) => {
    ...
    rsx! {
        div {
            ...
            div {
                ...
                // Tabs (only show My Badges/Pending/Created if authenticated)
                div {
                    class: "flex gap-2 px-4 pb-3 overflow-x-auto",
...
fn TabButton(
    ...
    rsx! {
        button {
            class: if active {
                "px-4 py-2 rounded-full bg-primary text-primary-foreground font-medium text-sm whitespace-nowrap flex items-center gap-2"
            } else {
                "px-4 py-2 rounded-full hover:bg-accent font-medium text-sm whitespace-nowrap flex items-center gap-2"
            },
            onclick: onclick,
            "{label}"
            if let Some(count) = badge_count {
                if count > 0 {
                    span {
                        class: "px-1.5 py-0.5 text-xs rounded-full bg-destructive text-destructive-foreground",
                        "{count}"
                    }
                }
            }
...
fn BadgeCard(badge: BadgeDefinition) -> Element {
    ...
    rsx! {
        Link {
            ...
            // Image
            div {
                class: "aspect-square bg-muted flex items-center justify-center p-4",
...
fn BadgeCardSkeleton() -> Element {
    rsx! {
        div {
            class: "bg-card border border-border rounded-xl overflow-hidden animate-pulse",
                        ...
                        spawn(async move {
                            match nip58::accept_badge(&coord, &event_id).await {
                                Ok(_) => {
                                    ...
                                    // Page will refresh on tab change
                                }
                                Err(e) => {
