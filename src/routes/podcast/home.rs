...
use crate::components::{
    icons, DisplayEpisode, PodcastAddFeedModal, PodcastEpisodeCard,
    PodcastEpisodeCardSkeleton, PodcastShow, PodcastShowCard, PodcastShowCardSkeleton,
};
    ...
    rsx! {
        div { class: "min-h-screen",
            div { class: "sticky top-0 z-20 bg-background/80 backdrop-blur-sm border-b border-border",
                div { class: "p-4",
                    h1 { class: "text-xl font-bold", "Podcasts" }
                }
                div { class: "px-4 pb-4",
                    div { class: "relative",
                        input {
                    ...
                    div { class: "flex items-center justify-between",
                        div { class: "flex gap-1.5",
                            for (value , label) in [("all", "All"), ("rss", "RSS/Podcast 2.0"), ("nostr", "Nostr Native")] {
                                button {
                                    class: if *selected_platform.read() == value { "px-3 py-1.5 rounded-full text-xs font-medium bg-primary text-primary-foreground" } else { "px-3 py-1.5 rounded-full text-xs font-medium bg-muted/50 hover:bg-muted text-muted-foreground" },
                ...
                if !submitted_query.read().is_empty() {
                    ...
                    match *active_tab.read() {
                        PodcastTab::Discover => rsx! {
                            DiscoverTab { platform: selected_platform.read().clone() }
                        },
                        PodcastTab::Recent => rsx! {
                            RecentFromSubscriptions { platform: selected_platform.read().clone() }
                        },
                        PodcastTab::Library => rsx! {
                            LibraryTab { platform: selected_platform.read().clone() }
                        },
                    }
                }
            }
...
fn DiscoverTab(props: DiscoverTabProps) -> Element {
    ...
    rsx! {
        div { class: "space-y-6",
            ...
            if props.platform != "nostr" {
                TrendingPodcasts { category: None }
            }
            ApiCategoryTiles {
...
fn CategoryTiles(props: CategoryTilesProps) -> Element {
    ...
    rsx! {
        div { class: "space-y-2",
            h3 { class: "font-semibold text-sm", "Browse by Category" }
            div { class: "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2",
                button {
                    class: if props.selected.is_none() { "p-2 rounded-lg text-center bg-primary text-primary-foreground text-xs font-medium transition" } else { "p-2 rounded-lg text-center bg-muted hover:bg-muted/80 text-foreground text-xs font-medium transition" },
                ...
                for cat in categories {
                    {
                        ...
                        rsx! {
                            button {
                                key: "{cat.name}",
                                class: if is_selected { "p-2 rounded-lg text-center bg-primary text-primary-foreground text-xs font-medium transition" } else { "p-2 rounded-lg text-center bg-muted hover:bg-muted/80 text-foreground text-xs font-medium transition" },
        ...
        div { class: "space-y-3",
            ...
            if *loading.read() {
                div { class: "flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide",
                    for i in 0..12 {
                        div { key: "{i}", class: "shrink-0 w-28 sm:w-32",
                    ...
                    div { class: "flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide",
                        for feed in feeds.iter() {
                            div {
                                ...
                                TrendingPodcastCard { feed: feed.clone() }
                            }
                        }
                    }
                }
    ...
    rsx! {
        div { class: "space-y-3",
            div { class: "flex items-center justify-between",
                h3 { class: "font-semibold",
                    "Browse Podcasts"
                    span { class: "text-xs text-muted-foreground font-normal ml-2", "({category_label})" }
                }
                if !is_loading && total_count > 0 {
                    span { class: "text-xs text-muted-foreground", "{total_count} podcast(s)" }
                }
            }
            if is_loading {
                div { class: "space-y-2",
...
fn BrowsePodcastRow(props: BrowsePodcastRowProps) -> Element {
    let image = props
        ...
        .clone()
        .unwrap_or_else(|| {
            format!("https://api.dicebear.com/7.x/shapes/svg?seed={}", props.title)
        });
    rsx! {
        Link {
            ...
            div { class: "flex items-center gap-1.5 shrink-0",
                if props.has_v4v {
                    span { class: "px-1.5 py-0.5 text-[10px] font-medium bg-amber-500/20 text-amber-500 rounded",
                        "V4V"
                    }
                }
                if props.is_nostr {
                    span { class: "px-1.5 py-0.5 text-[10px] font-medium bg-purple-500/20 text-purple-400 rounded",
                        "Nostr"
                    }
                } else {
                    span { class: "px-1.5 py-0.5 text-[10px] font-medium bg-green-500/20 text-green-400 rounded",
                        "RSS"
                    }
                }
            }
...
fn TrendingPodcastCard(props: TrendingPodcastCardProps) -> Element {
    ...
    rsx! {
        Link {
            ...
            div { class: "p-2",
                p { class: "text-xs font-medium line-clamp-2", "{feed.title}" }
    ...
    let category_buttons: Vec<(String, String, bool)> = if let Some(ref cats) = *cats_data {
        popular_cats
            ...
            .filter_map(|display_name| {
                cats.iter()
                    .find(|c| {
                        c.name.to_lowercase().contains(&display_name.to_lowercase())
                    })
                    .map(|api_cat| {
    ...
    rsx! {
        div { class: "space-y-2",
            ...
            if is_loading {
                ...
                div { class: "flex gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-muted",
                    button {
                        class: if props.selected.is_none() { "px-3 py-1.5 rounded-full text-xs font-medium bg-primary text-primary-foreground whitespace-nowrap shrink-0" } else { "px-3 py-1.5 rounded-full text-xs font-medium bg-muted hover:bg-muted/80 text-foreground whitespace-nowrap shrink-0" },
                        onclick: move |_| props.on_select.call(None),
                        "All"
                    }
...
fn RecentEpisodesTab(props: RecentEpisodesTabProps) -> Element {
    rsx! {
        RecentEpisodesMerged { platform: props.platform }
    }
}
/// Recent tab props
#[derive(Props, Clone, PartialEq)]
...
fn RecentFromSubscriptions(props: RecentFromSubscriptionsProps) -> Element {
    ...
    if !is_authenticated {
        return rsx! {
            div { class: "text-center py-16 space-y-4",
                div { class: "text-4xl", "🎧" }
                h3 { class: "text-lg font-semibold", "Sign in to see recent episodes" }
                ...
                for ep in live_episodes {
                    if let Some(feed_id) = ep.feed_id {
                        if podcast_ids_set.contains(&feed_id) {
                            if let Ok(feed) = podcast_index::get_podcast_by_id(feed_id)
                                .await
                            {
                                all_episodes
                                    .push(
                                        DisplayEpisode::from_podcast_index_live_episode(&ep, &feed),
                                    );
                            }
                        }
                    }
                ...
                .map(|sub| {
                    ...
                    async move {
                        ...
                        let display_episodes: Vec<DisplayEpisode> = episodes
                            .iter()
                            .map(|ep| DisplayEpisode::from_podcast_index_episode(
                                ep,
                                &feed,
                            ))
                            .collect();
        ...
        spawn(async move {
            ...
            let subscribed_pubkeys: Vec<String> = nostr_coords
                ...
                .filter_map(|coord| {
                    ...
                    if parts.len() >= 2 { Some(parts[1].to_lowercase()) } else { None }
                })
                .collect();
    ...
    let is_initial_loading = !client_ready || !has_signer || !subs_loaded || subs_loading
        ...
        || (show_nostr && *nostr_loading.read() && nostr_episodes.read().is_none()
            && !show_rss);
    ...
    if merged_episodes.is_empty() {
        ...
        return rsx! {
            div { class: "text-center py-16 space-y-4",
                div { class: "text-4xl", "📭" }
                h3 { class: "text-lg font-semibold", "No recent episodes" }
                p { class: "text-muted-foreground text-sm max-w-sm mx-auto",
            ...
            if podcast_id.is_some() || podcast_guid.is_some() {
                ...
                if let Some(id) = podcast_id {
                    match podcast_index::get_podcast_by_id(id).await {
                        Ok(feed) => {
                            return Some(SubscriptionMetadata::Rss(Box::new(feed)));
                        }
                        Err(e) => {
                            log::warn!(
                                "Failed to fetch RSS podcast by ID {}: {}", id, e
                            );
                        }
                    }
                }
                if let Some(ref guid) = podcast_guid {
                    match podcast_index::get_podcast_by_guid(guid).await {
                        ...
                        Err(e) => {
                            log::warn!(
                                "Failed to fetch RSS podcast by GUID {}: {}", guid, e
                            );
                            return None;
                        }
                    }
                    ...
                    if let Ok(pubkey) = nostr_sdk::PublicKey::from_hex(pubkey_hex) {
                            ...
                            .await
                        {
                            if let Some(event) = events.into_iter().next() {
                                if let Ok(metadata) = podcast::parse_podcast_metadata(
                                    &event,
                                ) {
    ...
    if let Some(route) = route {
        rsx! {
            Link {
                ...
                class: "flex items-center gap-3 p-2 rounded-lg hover:bg-muted transition",
                if is_loading {
                    div { class: "w-12 h-12 rounded bg-muted animate-pulse" }
        ...
        rsx! {
            div { class: "flex items-center gap-3 p-2 rounded-lg opacity-50",
                div { class: "w-12 h-12 rounded bg-muted flex items-center justify-center text-muted-foreground",
                    "{icon}"
                }
                div { class: "flex-1 min-w-0",
                    div { class: "font-medium truncate", "{message}" }
                }
            }
        }
    }
...
fn PodcastSearchResults(props: PodcastSearchResultsProps) -> Element {
    ...
    let api_search = use_resource(move || {
        ...
        async move {
            ...
            match podcast_index::search_podcasts(&q, Some(20)).await {
                Ok(feeds) => {
                    log::info!(
                        "API search for '{}' returned {} results", q, feeds.len()
                    );
                    Ok(feeds)
                }
                Err(e) => {
    ...
    rsx! {
        div { class: "space-y-4",
            div { class: "flex items-center justify-between",
                h3 { class: "font-semibold", "Search Results" }
                if has_any_data {
            ...
            if is_initial_loading {
                ...
                if nostr_still_loading && platform_display != "rss" {
                    div { class: "text-xs text-muted-foreground text-center py-2", "Searching Nostr..." }
                }
            }
        }
    }
    ...
    rsx! {
        Link {
            ...
            div { class: "flex-1 min-w-0",
                div { class: "font-medium truncate", "{props.title}" }
                if let Some(ref author) = props.author {
                    div { class: "text-sm text-muted-foreground truncate", "{author}" }
                }
            }
            div { class: "flex items-center gap-2 shrink-0",
                if props.has_v4v {
                    span { class: "px-1.5 py-0.5 text-[10px] font-semibold bg-amber-500/20 text-amber-500 rounded",
                        "V4V"
                    }
                }
                match props.source {
                    SearchResultSource::Rss => rsx! {
                        span { class: "px-1.5 py-0.5 text-[10px] font-semibold bg-green-500/20 text-green-500 rounded",
                            "RSS"
                        }
                    },
                    SearchResultSource::Nostr => rsx! {
                        span { class: "px-1.5 py-0.5 text-[10px] font-semibold bg-purple-500/20 text-purple-500 rounded",
                            "Nostr"
                        }
                    },
                }
...
fn NostrPodcastShows(props: NostrPodcastShowsProps) -> Element {
    ...
    if !*nostr_client::CLIENT_INITIALIZED.read()
        || (*loading.read() && shows.read().is_none())
    {
        return rsx! {
            div { class: "space-y-1",
                for i in 0..5 {
                    PodcastShowCardSkeleton { key: "{i}" }
                }
            }
        };
    }
    if let Some(err) = error.read().as_ref() {
        return rsx! {
            div { class: "text-center py-8 text-destructive", "Failed to load podcasts: {err}" }
        };
    }
...
fn RecentNostrEpisodes(props: RecentNostrEpisodesProps) -> Element {
    ...
    if !*nostr_client::CLIENT_INITIALIZED.read()
        || (*loading.read() && episodes.read().is_none())
    {
        return rsx! {
            div { class: "space-y-1",
                for i in 0..5 {
    ...
    if let Some(err) = error.read().as_ref() {
        return rsx! {
            div { class: "text-center py-8 text-destructive", "Failed to load episodes: {err}" }
        };
    }
    ...
    if let Some(eps) = episodes_read.as_ref() {
        ...
        if display_eps.is_empty() {
            rsx! {
                div { class: "text-center py-8 text-muted-foreground", "No episodes found yet." }
            }
        } else {
            rsx! {
                div { class: "space-y-1",
        ...
        use_effect(move || {
            ...
            spawn(async move {
                ...
                let fetch_futures: Vec<_> = subscriptions
                    ...
                    .collect();
                let results = futures::future::join_all(fetch_futures).await;
                let all_episodes: Vec<DisplayEpisode> = results
                    .into_iter()
                    .flatten()
                    .flatten()
                    .collect();
                log::info!(
                    "Fetched {} RSS episodes from subscriptions", all_episodes.len()
                );
                rss_episodes.set(Some(all_episodes));
                rss_loading.set(false);
            });
    ...
    let is_initial_loading = match props.platform.as_str() {
        ...
        "nostr" => {
            !*nostr_client::CLIENT_INITIALIZED.read()
                || (*nostr_loading.read() && !has_nostr_data)
        }
        _ => *rss_loading.read() && !has_any_data,
    };
    ...
    if display_eps.is_empty() {
        ...
        rsx! {
            div { class: "text-center py-8 text-muted-foreground space-y-3",
                p { "{platform_msg}" }
                if nostr_still_loading {
                    div { class: "text-xs text-muted-foreground", "Loading Nostr episodes..." }
                }
            }
        }
    } else {
    ...
    if rss_feed_ids.is_empty() {
        return rsx! {
            div { class: "bg-muted/30 rounded-lg p-4",
                h3 { class: "font-semibold mb-2", "Your Subscriptions" }
                p { class: "text-sm text-muted-foreground",
...
fn SubscribedFeedCard(props: SubscribedFeedCardProps) -> Element {
    let mut podcast_info = use_signal(|| {
        ...
        spawn(async move {
            if let Ok(id) = id_str.parse::<u64>() {
                match podcast_index::get_podcast_by_id(id).await {
                    ...
                    Err(e) => {
                        log::warn!(
                            "Failed to fetch podcast info for ID {}: {}", id_str, e
                        );
                        podcast_info
                            .set(Some((format!("Podcast #{}", id_str), None, None)));
                    }
                }
            } else {
    ...
    if *is_loading.read() {
        return rsx! {
            div { class: "flex items-center gap-3 p-3 bg-muted/30 rounded-lg animate-pulse",
    ...
    rsx! {
        div { class: "flex items-center gap-3 p-3 bg-muted/30 rounded-lg hover:bg-muted/50 transition",
            ...
            div { class: "flex-1 min-w-0",
                ...
                if let Some(ref auth) = author {
                    p { class: "text-sm text-muted-foreground truncate", "{auth}" }
                }
            }
            button {
    ...
    let shows: Vec<PodcastShow> = shows_by_pubkey
        ...
        .filter_map(|(pubkey, (_title, _created_at, image))| {
            ...
            Some(PodcastShow {
                ...
                description: Some(
                    "Podcast discovered from episodes. Metadata not yet published."
                        .to_string(),
                ),
...
async fn fetch_recent_nostr_episodes() -> std::result::Result<
    Vec<DisplayEpisode>,
    String,
> {
    let filter = Filter::new().kind(Kind::from(podcast::KIND_PODCAST_EPISODE)).limit(50);
    ...
    if parse_errors > 0 {
        log::info!(
            "Parsed {} episodes, {} parse failures", episodes.len(), parse_errors
        );
    } else {
...
async fn search_nostr_podcasts(
    query: &str,
) -> std::result::Result<Vec<PodcastShow>, String> {
    use nostr_sdk::SingleLetterTag;
    let filter = Filter::new()
        .kind(Kind::from(podcast::KIND_APP_DATA))
        .custom_tag(SingleLetterTag::from_char('d').unwrap(), "podcast-metadata")
