...
pub struct Repository {
    ...
    /// Zap split configuration: (pubkey_hex, relay_url, weight)
...
impl Repository {
    ...
    pub fn from_event(event: &Event) -> Option<Self> {
        ...
        for tag in event.tags.iter() {
            ...
            // Parse zap split tags: ["zap", pubkey, relay, weight]
            if tag_vec.len() >= 4 && tag_vec[0] == "zap" {
                if let Ok(weight) = tag_vec[3].parse::<u32>() {
                    zap_splits.push((tag_vec[1].to_string(), tag_vec[2].to_string(), weight));
                }
                continue;
            }
