    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ValidationError::NoIngredients => {
                write!(f, "At least one ingredient is required")
            }
            ValidationError::NoDirections => {
                write!(f, "At least one direction step is required")
            }
            ValidationError::InvalidDirectionFormat(msg) => {
                write!(f, "Invalid direction format: {}", msg)
            }
            ValidationError::ChefNotesTooLong => {
                write!(f, "Chef's notes exceed character limit")
            }
            ValidationError::PrepTimeTooLong => {
                write!(f, "Prep time exceeds character limit")
            }
            ValidationError::CookTimeTooLong => {
                write!(f, "Cook time exceeds character limit")
            }
            ValidationError::ServingsTooLong => {
                write!(f, "Servings exceed character limit")
            }
