...
struct StationContent {
    #[serde(default)]
    #[allow(dead_code)] // Needed for serde deserialization
    description: String,
    #[serde(default)]
...
impl RadioStation {
    ...
    pub fn from_event(event: &Event) -> Result<Self, String> {
        ...
        // Parse streams from stream tags
        ...
        // Fallback: parse streams from event content JSON (wavefunc compatibility)
        if streams.is_empty() {
        ...
        // Parse languages from lowercase "l" tags
...
pub async fn fetch_radio_stations(
    ...
    // Use aggregated fetch pattern: DB first, relay background sync
...
pub async fn search_radio_stations(query: &str, limit: usize) -> Result<Vec<RadioStation>, String> {
    ...
    match nostr_client::fetch_events_aggregated(filter, Duration::from_secs(5)).await {
        Ok(events) => {
            ...
            // Sort by relevance (stations with query in name first, then by created_at)
        ...
        Err(e) => {
            ...
            // Fallback: fetch all stations and filter client-side
            let filter = Filter::new()
                .kind(Kind::Custom(KIND_RADIO_STATION))
                .limit(200);
...
pub fn select_best_stream(streams: &[RadioStream]) -> Option<&RadioStream> {
    // Primary stream first
    if let Some(primary) = streams.iter().find(|s| s.is_primary) {
        return Some(primary);
    }

    // Highest bitrate non-HLS for compatibility
...
// Stream ranking constants (matching wavefunc scoring)
...
pub fn stream_score(stream: &RadioStream) -> u32 {
    ...
    // Primary streams get highest priority
    ...
    // Add bitrate bonus (normalized to 0-50 points, capped at 320kbps)
