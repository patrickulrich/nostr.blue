...
pub async fn decrypt_private_tags(event: &Event) -> Result<Vec<Tag>, String> {
    ...
    // Parse as JSON array of tag arrays
    ...
    let tags: Vec<Tag> = tag_arrays
        ...
        .filter_map(|arr| {
            ...
            // Reconstruct tag based on tag type
            match arr[0].as_str() {
                "p" if arr.len() >= 2 => PublicKey::parse(&arr[1]).ok().map(Tag::public_key),
                // Add other tag types as needed (e.g., "e" for events, "t" for hashtags)
                _ => None,
            }
        })
        .collect();
...
pub async fn create_people_list(
    ...
    // For private lists, encrypt empty array to establish the pattern
    ...
    // Sign the event first so we can return it
    let event = client
