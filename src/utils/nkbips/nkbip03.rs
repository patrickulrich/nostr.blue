...
static CITATION_MARKUP_REGEX: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
            r"citation::(end|foot|foot-end|inline|quote|prompt-end|prompt-inline)::(n(?:event|ote|addr)[a-zA-Z0-9]+)",
        )
                ...
                match style {
                    Some(CitationStyle::Inline) | Some(CitationStyle::PromptInline) => {
                        let display = citation_data
                            .map(|c| c.inline_text())
                            .unwrap_or_else(|| {
                                format!("[{}]", truncate_identifier(identifier))
                            });
                        ...
                        let attribution = citation_data
                            .map(|c| {
                                ...
                                if !c.author.is_empty() {
                                    ...
                                    if !c.title.is_empty() {
                                        attr.push_str(
                                            &format!(", <cite>{}</cite>", html_escape(&c.title)),
                                        );
                                    }
...
pub fn generate_footnotes_html_with_data(
    ...
    let mut html = String::from(
        "<section class=\"citation-footnotes\" aria-label=\"Footnotes\"><hr class=\"citation-divider\"><ol class=\"citation-list\">",
    );
        ...
        let type_badge = citation_data
            .map(|c| match c.citation_type {
                CitationType::Internal => {
                    "<span class=\"citation-type-badge citation-type-internal\" title=\"Nostr reference\">📌</span>"
                }
                CitationType::ExternalWeb => {
                    "<span class=\"citation-type-badge citation-type-external\" title=\"Web reference\">🌐</span>"
                }
                CitationType::Hardcopy => {
                    "<span class=\"citation-type-badge citation-type-hardcopy\" title=\"Book/Journal\">📖</span>"
                }
                CitationType::Prompt => {
                    "<span class=\"citation-type-badge citation-type-prompt\" title=\"AI citation\">🤖</span>"
                }
            })
            .unwrap_or("");
        html.push_str(
...
pub fn generate_endnotes_html_with_data(
    ...
    let mut html = String::from(
        "<section class=\"citation-endnotes\" aria-label=\"References\"><h2 class=\"citation-heading\">References</h2><ol class=\"citation-list\">",
    );
