...
pub fn parse_event_kinds_from_readme(content: &str) -> Vec<EventKindInfo> {
    ...
    for line in content.lines() {
        // Detect the Event Kinds section
        if line.contains("## Event Kinds") {
            in_event_kinds_section = true;
            continue;
        }

        // Detect end of Event Kinds section (next ## header)
        ...
        // Only parse rows after the header separator
        if !found_header_separator {
            continue;
        }

        // Parse table rows like: | `0` | User Metadata | [01](01.md) |
        if line.starts_with("|") && line.contains("|") {
            ...
            if parts.len() >= 4 {
                ...
                // Extract NIP reference (just the number)
...
fn extract_nip_number(text: &str) -> String {
    ...
    // Fallback: just extract any 2-char alphanumeric sequence
