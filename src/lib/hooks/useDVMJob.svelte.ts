...
export function useDVMJob() {
	...
	const submitJob = createMutation(() => ({
			...
			queryFn: async ({ signal }) => {
				...
				try {
					...
					const events = await loadWithRouter({
						filters: [{ kinds: [resultKind], '#e': [idHex], limit: 50 }],
						signal
					});
		...
		return createQuery(() => ({
			...
			queryFn: async ({ signal }) => {
				...
				try {
					const idHex = normalizeEventId(jobRequestId);
					const events = await loadWithRouter({
						filters: [{ kinds: [7000], '#e': [idHex], limit: 20 }],
						signal
					});
		...
		return createQuery(() => ({
			...
			queryFn: async ({ signal }) => {
				try {
					...
					const events = await loadWithRouter({
						filters: [{ kinds: [resultKind], authors: [authorHex], limit: 50 }],
						signal
					});
